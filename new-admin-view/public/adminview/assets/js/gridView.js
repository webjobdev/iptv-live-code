    ...
    */
    ...
    deleteRecords: function (scope, http, id) {
        ...
        scope.deleteRequest = scope.request.post(scope.request.getUrl(scope.routeName + '/action'), angular.extend({}, { selectedCheckbox: id }, scope.requestParams), function (data) {
            ...
            scope.request.setToaster('success', data.message);
...
var gridView = ['$document', 'requestFactory', function ($document, requestFactory) {
    ...
    return {
        ...
        controller: ['$http', '$scope', '$controller', '$document', '$attrs', function ($http, $scope, controller, $document, $attrs) {
            ...
            $scope.recordsName = $attrs.recordsName || 'records';
            $scope.currentPage = 1;
            $scope.totalRecords = '';
            $scope[$scope.recordsName] = '';
            $scope.links = '';
            $scope.fieldName = '';
            $scope.sortOrder = '';
            $scope.showRecords = false;
            ...
            $scope.selectedRecords = [];
            $scope.gridBulkActionsFlag = {
                'isDeactivateBulkRecord': false,
                'isActivateBulkRecord': false,
                'isDeleteBulkRecord': false,
                'statusUpdateSingleRecord': false,
            };
            ...
            */
            ...
            $scope.successCallback = function (response) {
                ...
                if (response.data.total > 0) {
                    ...
                    $scope.noRecords = false;
                    $scope[$scope.recordsName] = response.data.data;
                    $scope.showGridError = true;
                    $scope.gridError = '';
            ...
            */
            $scope.updateStatus = function (record) {
                $scope.gridBulkActionsFlag.statusUpdateSingleRecord = true;
                var status = record.is_active == 1 ? 0 : 1;
                record.is_active = status;
                var ids = [];
                ids.push(record.id);
                $scope.request.post(
                    $scope.request.getUrl($scope.routeName + '/update-status'),
                    angular.extend({}, { selectedCheckbox: ids, status: status }, $scope.requestParams),
                    function () {
                        record.is_active = status;
                        $scope.getRecords(true);
                    }
            ...
            */
            ...
            /**
            * Method to Close the SidePanel for Grids
            */
            $scope.gridSideFormClose = function () {

                $(".sidepanel").removeClass("in");
            }

            $scope.gridSideFormOpen = function () {

                $(".sidepanel").addClass("in");
            }

            /**
            * Method to select bulk records in the grid to delete, activate and
            * deactivate records
            *
            * @return array
            */
            $scope.selectBulkRecords = function () {
                if (angular.element('#selectall').prop('checked')) {
                    angular.element('.checkbox').each(function () {
                ...
                return $scope.selectedRecords;
            }
            /**
            * Method to enable bulk drop-down when the checkbox along
            * each record is checked
            *
            * @return void
            */
            $scope.selectRecord = function ($event, id) {
                ...
                /** If all the checkbox are checked in a grid page, then check/uncheck
                * 	master checkbox
                */
                gridObj.checkMasterCheckbox();
            }
            /**
            * Function to display the confirmation popup for bulk action in the grid
            *
            * @return void
            */
            $scope.confirmationPopupBulkAction = function (action, status = null) {
                if (action == 'delete-Popup') {
                    $scope.confirmationPopupBulkDelete();
                } else {
                    $scope.confirmationPopupBulkActivateOrDeactivate(status);
                }
            }
            /**
            * Function to display the confirmation popup for bulk delete action in the grid
            *
            * @return void
            */
            $scope.confirmationPopupBulkDelete = function () {
                $scope.deleteParams = $scope.selectedRecords;
                $scope.gridBulkActionsFlag.isDeactivateBulkRecord = false;
                $scope.gridBulkActionsFlag.isActivateBulkRecord = false;
                $scope.gridBulkActionsFlag.isDeleteBulkRecord = true;
                $scope.gridBulkActionsFlag.statusUpdateSingleRecord = false;
            }
            /**
            * Function to display the confirmation popup for bulk activate/deactivate action in the grid.
            *
            * @return void
            */
            $scope.confirmationPopupBulkActivateOrDeactivate = function (status) {
                $scope.activateParams = $scope.selectedRecords
                if (status == 'activate') {
                    $scope.gridBulkActionsFlag.isDeleteBulkRecord = false;
                    $scope.gridBulkActionsFlag.isDeactivateBulkRecord = false;
                    $scope.gridBulkActionsFlag.isActivateBulkRecord = true;
                    $scope.gridBulkActionsFlag.statusUpdateSingleRecord = false;
                } else if (status == 'deactivate') {
                    $scope.gridBulkActionsFlag.isDeleteBulkRecord = false;
                    $scope.gridBulkActionsFlag.isActivateBulkRecord = false;
                    $scope.gridBulkActionsFlag.isDeactivateBulkRecord = true;
                    $scope.gridBulkActionsFlag.statusUpdateSingleRecord = false;
                }
            }
            /**
            * Method to call the bulk status update action
            *
            * @return void
            */
            $scope.confirmBulkStatusUpdate = function (status) {
                if (status == 1) {
                    $scope.gridBulkActionsFlag.isActivateBulkRecord = false;
                } else if (status == 0) {
                    $scope.gridBulkActionsFlag.isDeactivateBulkRecord = false;
                }
                $scope.updateBulkStatus($scope.activateParams, status);
            }
            /**
            * Method to bulk  update the status in grid
            *
            * @return void
            */
            $scope.updateBulkStatus = function (ids, status) {
                $scope.activateParams = '';
                $scope.showRecords = false;
                $scope.gridLoadingBar = true;
                var totalRecords = ids.length;
                $scope.deleteRequest = $scope.request.post($scope.request.getUrl($scope.routeName + '/update-status'),
                    angular.extend({}, { selectedCheckbox: ids, status: status }, $scope.requestParams), function (data) {
                        $scope.responseMessage = data.message;
                        $scope.showResponseMessage = true;
                        $scope.selectedRecords = [];
                        angular.element('#selectall').removeAttr('checked');
                        if ($scope.records.length - totalRecords > 0) {
                            $scope.getRecords(true);
                        } else {
                        ...
                        $scope.request.setToaster('success', data.message);
                        $scope.request.getToaster();
                    });
            }
            /**
            * Function to display the confirmation popup for single
            * status update action in the grid
            *
            * @param object record
            * @return void
            */
            $scope.confirmationPopupSingleRecordAction = function (record) {
                $scope.gridBulkActionsFlag.statusUpdateSingleRecord = true;
                $scope.statusParams = record;
            }
            /**
            * Method to update the status of single record
            *
            * @return void
            */
            $scope.confirmSingleStatusUpdate = function () {
                $scope.updateStatus($scope.statusParams);
            }
            /**
            * Method to close the side panel form
            *
            * @return void
            */
            $scope.closesidePanelForm = function () {
                classie.remove(document.getElementsByClassName('sidepanel')[0], 'in');
            }
