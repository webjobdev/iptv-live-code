  ...
  function (
    ...
    */
        ...
        requestFactory.post(
          ...
          function (response) {
            scope.selectedVideo.is_active = scope.selectedVideo.is_active
              ? true
              : false;
            requestFactory.setToaster('success', 'Radio updated successfully');
    ...
    this.saveLiveVideo = function ($event) {
      ...
      if (baseValidator.validateAngularForm($event.target, scope)) {
        ...
        if (this.videoSubmitted == false) {
          ...
          requestFactory.post(
            requestFactory.getUrl('createlivestream'),
            scope.selectedVideo,
            function (response) {
              requestFactory.setToaster('success', response.message);
    ...
    $(document).ready(function () {
      ...
      /*
 * Mobile Post Image Upload Part
 */
      var mobilePosterImage = document.getElementById('mobile_poster_image');
      $(document).on('change', '.uploadMobilePosterImg', function (e) {
        var videoItem = $(this).data("video-index");
        scope.errors[videoItem] = {};
        var ValidImageTypes = ["image/jpeg", "image/png"];
        var files = e.target.files;
        ...
        if ($.inArray(fileType, ValidImageTypes) < 0) {
          scope.$apply();
          // BEGIN : To show invalid error message in the croppre box
          $('#mobile_poster_modal').modal('show');
          $('.crop-body').hide();
          $('#submit_mobile_poster_image').hide();
          $('.mobile_poster_error_msg').show().text("Invalid file format. Upload only jpeg and png file formats, click cancel to continue");
          // END : To show invalid error message in the croppre box
          return;
        }
        $('.crop-body').show();
        ...
        readAsMobilePosterUrl(this, videoIndex);
      });
      var cropBoxImgData;
      var canvasImgData;
      var cropperImg;
      ...
      $(document).on('show.bs.modal', '#mobile_poster_modal', function () {
        $('#submit_mobile_poster_image').show();
        $('.mobile_poster_error_msg').hide();
        setTimeout(function () {
          cropperImg = new Cropper(mobilePosterImage, {
            autoCropArea: 1,
            viewMode: 1,
            aspectRatio: 380 / 500,
            ...
            ready: function () {
              //Should set crop box data first here
              var config = { left: 0, top: 0, width: 380, height: 500 };
              cropperImg.setCropBoxData(config).setCanvasData(canvasImgData);
            }
          });
        }, 500);
      ...
      $(document).on('hidden.bs.modal', '#mobile_poster_modal', function () {
        document.getElementsByClassName("uploadMobilePosterImg")[0].value = "";
        $('#submit_mobile_poster_image').prop('disabled', false);
        cropperImg.destroy();
      });
      $(document).on('click', '#submit_mobile_poster_image', requestFactory.access_token, function () {
        cropBoxImgData = cropperImg.getCropBoxData();
        canvasImgData = cropperImg.getCroppedCanvas().toBlob(function (blob) {
          var formImgData = new FormData();
          formImgData.append('module', 'video');
          formImgData.append('size', 'mobile_posters');
          formImgData.append('image', blob);
          $('.crop-body').hide();
          $('.poster_loader-container').show();
          $('#submit_poster_image').prop('disabled', true);
          $.ajax($('meta[name="base-api-url"]').attr('content') + '/videos/mobileposter', {
            method: "POST",
            data: formImgData,
            processData: false,
            contentType: false,
            ...
            success(data) {
              ...
              $('.uploaded_mobile_poster_img').attr('src', data.info);
              $('.uploaded_mobile_poster_img').show();
              scope.selectedVideo.mobile_poster_image = data.info;
              scope.selectedVideo.is_mobile_posterimg_updated = 1;
              scope.$apply();
              $('.poster_loader-container').hide();
            ...
            error() {
              ...
              $('.mobile_poster_error_msg').show().text("Please upload bigger image, click cancel to continue");
            },
          })
        }, 'image/jpeg');
      });
    ...
    function readAsMobilePosterUrl(input, videoIndex) {
      if (input.files && input.files[0]) {
        var readerImg = new FileReader();
        readerImg.onload = function (e) {
    ...
    this.deleteRecordsVideos = function (id, videoStatus) {
      ...
      scope.deleteRequest = requestFactory.post(
        ...
        function (data) {
          ...
          if (
            ($('.not-saved').length <= 0 || result.length <= 0) &&
            currentUrl.includes('livevideos')
          ) {
            $window.location = requestFactory.getTemplateUrl(
              'admin/livevideos'
            );
    ...
    scope.validateTrailer = function (file, videoItem) {
      angular.forEach(file, function (value, key) {
        if (value) {
          ...
          if (extension === 'mp4' && fileSize < 1024) {
...
function videoUploader(scope, requestFactory) {
  ...
  this.initializeFineUploader = function () {
    window.fineUploader = new qq.FineUploaderBasic({
      ...
      request: {
        endpoint:
          window.VPlay.route.videoUploadEndpoint + 'api/media/admin/upload-video',
        customHeaders: {
