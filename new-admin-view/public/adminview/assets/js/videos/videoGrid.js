...
var VideoGridController = ['flowFactory', '$scope', 'requestFactory', '$rootScope', '$window', '$sce', '$timeout', '$compile', '$interval',
   ...
   */
    ...
    // upload video data and edit data
    ...
    * */
    ...
    requestFactory.post(
      requestFactory.getUrl('liveevents/records'), this.defineProperties,
      function (response) {
        scope.records = response.data.data;
        scope.organizationCounts = {};

        if (Array.isArray(scope.records)) {
          scope.records.forEach(function (record) {
            if (record.is_live == 3) {
              var orgName = record.organization || '-';
              // console.log(orgName);            
              if (!scope.organizationCounts[orgName]) {
                scope.organizationCounts[orgName] = 1;
              } else {
                scope.organizationCounts[orgName]++;
              }
            }
          });
        }
    ...
    scope.calculateDays = function (start, end) {
      if (!start || !end) {
        // console.log("Missing start or end date:", { start, end });
        return '';
      }
      ...
      //   console.log("Start and End are the same day: Today");
      //   return 'Today';
      // }

      if (today.getTime() === startDate.getTime()) {
        // console.log("Current date and start date is same");
        return 'Live';
      }

      if (today < startDate) {
        // console.log("Current date is before start date: Live Event Start Soon");
        return 'Live Event Start Soon';
      }

      if (today > endDate) {
        // console.log("Current date is after end date: Live Event Ended");
        return 'Live Event Ended';
      }

      if (today > startDate && today <= endDate) {
        console.log("Current date is between start and end date: Live");
        return 'Live';
      }
    ...
    this.togglePublishNow = function (record, id) {
      ...
      requestFactory.post(
        requestFactory.getUrl('liveevents/toggle-publish-now/' + id),
        payload,
        function (response) {
          requestFactory.getToaster();
          requestFactory.setToaster('success', 'Live Event Update.');
          setTimeout(() => {
            location.reload();
          }, 350);
        }
