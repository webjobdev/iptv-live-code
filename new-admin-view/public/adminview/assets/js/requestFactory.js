...
var requestFactory = ['$http', '$rootScope', '$filter', '$window', function (http, rootScope, $filter, $window) {
  ...
  var requestHandler = {
    ...
    hasPermission: function (permission) {
      let userPermissionsString = localStorage.getItem('user_permissions');
      if (!userPermissionsString) return false;

      let userPermissions;
      // Super Admin bypass - Direct check for "all" string
      if (userPermissionsString === '"all"' || userPermissionsString === 'all') {
        this.userpermissionList = 'all';
        return true;
      }
      ...
      try {
        ...
        console.error("Invalid JSON in user_permissions", e);
        return false;
      }

      this.userpermissionList = userPermissions;

      // Super Admin bypass - check various formats of "all" or "true" inside object
      if (userPermissions && (userPermissions === 'all' || userPermissions.all === true || userPermissions.all === 'true' || userPermissions.all == 1 || userPermissions.all === 'all')) {
        return true;
      }
      ...
      if (permission !== undefined && userPermissions) {
        if (Array.isArray(userPermissions)) {
          for (var i = 0; i < userPermissions.length; i++) {
            if (userPermissions[i].permission_module_name === permission) {
              return (userPermissions[i].view == 1 || userPermissions[i].view == '1');
            }
          }
          ...
          let res = userPermissions[permission];
          if (res && (res.view == 1 || res.view == '1' || res.View == 1 || res.View === true)) {
              return true;
          }
          return !!res;
        }
