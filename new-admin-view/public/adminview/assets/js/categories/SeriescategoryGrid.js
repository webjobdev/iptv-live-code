...
var SeriesCategoryGridController = ['$scope', '$rootScope', 'requestFactory', '$window', '$sce', '$timeout', '$compile', '$interval',
    function (scope, rootScope, requestFactory, $window, $sce, $timeout, $compile, $interval) {
        ...
        // this.category.is_mobile_image_updated = 0;
        ...
        this.fetchInfo = function () {
            requestFactory.get(requestFactory.getUrl('series-category/info'),
        ...
        this.categorySave = function ($event, id) {
            ...
            if (id) {
                requestFactory.post(
                    requestFactory.getUrl('series-category/edit/categorie/' + vodid),
                    this.category,
                ...
                requestFactory.post(
                    requestFactory.getUrl('series-category/add'),
                    this.category,
                    // console.log("✅ API Response:", this.category),
                    function (response) {
                        // scope.getRecords(true);
        ...
        this.Savecategory = function ($event, id) {
            ...
            if (id) {
                requestFactory.post(
                    requestFactory.getUrl('series-category/edit/categories/' + categorieEditId), this.category,
                    function (response) {
                        scope.getRecords(true);
                        requestFactory.setToaster('success', response.message);
                        requestFactory.getToaster();
                ...
                requestFactory.post(
                    requestFactory.getUrl('series-category/add/categorie'), payload,
                    function (response) {
                        scope.getRecords(true);
                        requestFactory.setToaster('success', response.message);
                        requestFactory.getToaster();
        ...
        this.SaveSubctgry = function ($event) {
            ...
            requestFactory.post(
                requestFactory.getUrl('series-category/add/sub-categorie'), payload,
                function (response) {
                    scope.getRecords(true);
                    requestFactory.setToaster('success', response.message);
                    requestFactory.getToaster();
        ...
        this.editCategory = function (records) {
            ...
            this.category.series_categorie_name = records.series_categorie_name;
        ...
        scope.onCategorySearch = function (event) {
            ...
            scope.records = scope.allRecords.filter(function (record) {
                return record.series_categorie_name &&
                    record.series_categorie_name.toLowerCase().includes(searchValue);
            });
            // }
        }
...
window.gridControllers = {
    SeriesCategoryGridController: SeriesCategoryGridController
};
