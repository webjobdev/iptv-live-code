...
var CategoryGridController = ['$scope', '$rootScope','requestFactory', '$window', '$sce', '$timeout', '$compile', '$interval', function (scope, rootScope, requestFactory, $window, $sce, $timeout, $compile, $interval) {
    ...
    this.fetchInfo = function () {
        requestFactory.get(requestFactory.getUrl('categories/info'), this.defineProperties, function (response) { 
    ...
    this.mobileUrl = function() {
        $timeout(function(){
            window.CategoryImageUploadHandler = new uploadHandler;
            window.CategoryImageUploadHandler.initate({
                file: 'mobile-category-image',
                previewer: 'mobile-category-image-preview',
                deleteIcon: 'mobile-category-image-delete',
                progress: 'mobile-category-image-progress',
                resolution:'220*230',
                isImageFileType:true,
                beforeUpload: function () {
                    if (!scope.$$phase) {
                ...
                afterUpload: function (response) {
                    self.category.mobileimage = response.info;
                    self.category.module = 'mobile-category-image';
                    self.category.is_mobile_image_updated = 1;
                }
            });
        },1000);
      };
    ...
    this.editCategory = function (records) {
        ...
        this.category.mobile_url = records.mobile_url;
        this.showcategory = (this.category.is_web_series == 1) ? false : true;
        this.category.is_image_updated = 0;
        this.category.is_mobile_image_updated = 0;
    ...
    this.categoryTranslateSave = function (event, id) {
        ...
        requestFactory.post(requestFactory.getUrl('categories/addLanguage/' + id), this.categoryTranslation, function (response) {
            requestFactory.setToaster('success', response.message);
            requestFactory.getToaster();
  ...
  this.checkMasterCheckbox = function () {
    ...
    this.deleteBulkRecords = function (id, status) {
        ...
        scope.deleteParams = '';
        scope.deleteRequest = requestFactory.post(requestFactory.getUrl('categories/action'), angular.extend({}, {
            selectedCheckbox: id,
            status: status
