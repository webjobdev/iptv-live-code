var uploadHandler = function(){
      ...
      if(this.isBrowserSupportedHtmlFive()){
        ...
        var opt = {
            ...
            locale      : {
              ...
              error : {
                minimumResolution : 'Image minimum resolution should be atleast 600*300',
                mime              : 'Image should be in :mime',
                fileSize          : 'Image size should be less than :maxSize MB',
                invalid           : 'Image selected is invalid, minimum resolution should be :resolution',                
        ...
        this.resolution = this.options.resolution ? this.options.resolution : '600*300';
  ...
  */    
  this.getXMLHttpRequest = function() {
    ...
    var uploadActionURL = window.VPlay.route.apiUrl+this.file.dataset.action;
    xhr.open("POST", uploadActionURL);
  ...
  */      
  this.previewFile = function(file,index,fileSrc,validationErrors) {
    ...
    /*
      @todo To hide image progress bar
      ...
      if(this.options.locale.error.hasOwnProperty(error)){
        ...
        switch(error){
          case 'minimumResolution':
            messgae = this.options.locale.error[error].replace(/:resolution/g, this.options.resolution);
          break;
          case 'mime':
  ...
  */     
  this.getErrorMessageFromResponse = function(response) {
    ...
    return (this.options.isImageFileType) ? 
    this.options.locale.error.invalid .replace(/:resolution/g, this.options.resolution)
    : this.options.locale.documenterror.invalid;
  };
  /**
