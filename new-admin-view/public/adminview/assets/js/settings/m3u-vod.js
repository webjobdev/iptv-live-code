...
var M3UVodController = ['$scope', 'requestFactory', '$window', '$sce', '$timeout', '$compile', '$interval', '$rootScope',
    function (scope, requestFactory, $window, $sce, $timeout, $compile, $interval, rootScope) {
        ...
        this.save = function ($event, id) {
            ...
            if (id) {
                requestFactory.post(
                    requestFactory.getUrl('m3u-vod/edit/' + id),
                ...
                requestFactory.post(
                    requestFactory.getUrl('m3u-vod/create'),
                    payload, function (response) {
                        scope.getRecords(true);
                        requestFactory.setToaster('success', response.message);
                        requestFactory.getToaster();
...
window.gridControllers = { M3UVodController: M3UVodController };