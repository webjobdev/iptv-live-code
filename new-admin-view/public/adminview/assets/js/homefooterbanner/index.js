  ...
  this.banner.is_mobile_image_updated = 0;
    scope.singleConfig= {
      ...
      load: function(query, callback) {
          ...
          $.ajax({
              url: $('meta[name="base-api-url"]').attr('content') + '/home-footer-banner/searchvideos?search=' + query,
...
scope.clearbannerImage = function(){
    ...
    this.save = function ($event,id) {
      ...
      if(this.banner.banner_image != '' && this.banner.banner_url != '' && this.banner.mobile_image != '' && this.banner.mobile_url != '' ) {
        ...
        if (baseValidator.validateAngularForm($event.target,scope) && !scope.errors.hasOwnProperty('banner_url') && !scope.errors.hasOwnProperty('mobile_url')) {
            var Apiurl = (id) ? 'home-footer-banner/edit/' + id : 'home-footer-banner/add';
  ...
  this.fetchInfo = function() {
      requestFactory.get(requestFactory.getUrl('home-footer-banner/info'),this.defineProperties,function(response){
  ...
  this.mobileUrl = function() {
    ...
    this.deleteBannerImage = function () {
        requestFactory.toggleLoader();
        requestFactory.post(requestFactory.getUrl('home-footer-banner/delete-banner-image/' + this.banner.id), this.banner, function (response) {
            requestFactory.toggleLoader();
            requestFactory.setToaster('success', response.message);
            requestFactory.getToaster();
            scope.getRecords(true);
