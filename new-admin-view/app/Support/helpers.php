...
if (!function_exists('getBaseAssetsUrl')) {
    function getBaseAssetsUrl($path = '') {
...
if (!function_exists(('check_menu_flag'))) {
    function check_menu_flag($permission, $child = 0) {
        $showMenu = 0;
        if(!empty(Auth::user())) {
            $userPermission = json_decode(Auth::user()->group()->first()->toArray()['permissions'], true);
            $moduleArray = array_keys($userPermission);

            if(!$child) {
                if(in_array($permission, $moduleArray)) {
                    $showMenu = 1;
                }
            }
            else {
                $found = array_filter($userPermission,function($subModule) use ($permission){
                    if(in_array($permission, $subModule)) {
                        return $subModule;
                    }
                }); 
