        ...
        <table class="table" id="fixTable">
            <thead>
                <tr>
                    @include('audio::admin.common.bulkActionLayout', [
                        'access_type' => 'admin_user_all_write',
                    ])
            ...
            <tbody>
                <tr class="search_text">
                    ...
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.name"
                            ...
                            data-original-title="{{ trans('user::adminuser.enter_user_name') }}">
                    </td>
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.email"
                            data-boot-tooltip="true" data-toggle="tooltip"
                            data-original-title="{{ trans('user::adminuser.enter_email') }}">
                    </td>
                    <td></td>
                    <td>
                ...
                <tr data-ng-if="noRecords">
                    <td colspan="@{{ heading.length + 1 }}" class="no-data center">{{ trans('base::general.not_found') }}
                    </td>
                </tr>
                <tr data-ng-if="showRecords" data-ng-repeat="record in records track by $index"
                    ...
                    <td>
                        ...
                        <div ng-if="record.id != authId && checkAccess('admin_user_all_write')">
                            <div class="tooltip-parent">
                                <span class="status-active"
                                    ng-if="record.is_active == 1 && record.group.is_deletable == 1"
                            ...
                            <div class="tooltip-parent">
                                <span class="status-active"
                                    ng-if="record.is_active == 1 && record.group.is_deletable == 0"
                                    data-boot-tooltip="true">{{ trans('user::user.message.active') }}</span>
                                <span
                                    class="tooltip_title">{{ trans('user::user.cannot_deactivate_superadmin') }}</span>
                            </div>
                            <div class="tooltip-parent">
                                <span class="status-inactive" ng-if="record.is_active != 1" style="cursor: pointer;"
                                    data-toggle="modal" data-target="#single-record-status-update-popup"
                                    data-ng-click="confirmationPopupSingleRecordAction(record)"
                        ...
                        <div ng-if="record.id == authId && checkAccess('admin_user_all_write')">
                            <div class="tooltip-parent">
                                <button disabled class="status-active"
                                    ng-if="record.is_active == 1 && record.group.is_deletable == 1"
                                    style="cursor: pointer;"><span
                                        data-boot-tooltip="true">{{ trans('user::user.message.active') }}</span></button>
                            ...
                            <div class="tooltip-parent">
                                <button disabled class="status-inactive" ng-if="record.is_active != 1"
                                    style="cursor: pointer;"><span
                                        data-boot-tooltip="true">{{ trans('user::user.message.inactive') }}</span></button>
                                <span class="tooltip_title">{{ __('video::videos.permission_denied') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flexbox align-items-center justify-center">
                            <div ng-if="record.id != usrCtrl.authId && checkAccess('admin_user_all_write')">
                                <div class="column edit_table_icon tooltip-parent"
                                    data-ng-if="record.group.is_deletable == 1">
                                    <button class="table_action sidepanel-open"
                                        data-ng-click="usrCtrl.editUser(record)">
                                ...
                                <div class="tooltip-parent">
                                    <span ng-mouseover="getTooltip($event)"
                                        data-ng-if="record.id != usrCtrl.authId || record.group.is_deletable == 1"
...
<div class="sidepanel">
    ...
    <div class="pop_over_continer form-page">
        <form name="userForm" method="POST" data-base-validator
            data-ng-submit="usrCtrl.save($event, usrCtrl.user.id)" enctype="multipart/form-data">
            {!! csrf_field() !!}
            <div class="sidepanel-header flexbox align-items-center">
                <h5 data-ng-if="!usrCtrl.user.id">{{ trans('user::adminuser.user_heading') }} -
                    {{ trans('user::adminuser.add_new_user') }}</h5>
                <h5 data-ng-if="usrCtrl.user.id">{{ trans('user::adminuser.user_heading') }} -
                    {{ trans('user::adminuser.edit_new_user') }}</h5>
            </div>
            ...
            <div class="sidepanel-scroll">
                ...
                <div class="form-group" data-ng-class="{'has-error': errors.email.has}">
                    ...
                    <div class="form-input">
                        <input type="text" data-ng-if="!usrCtrl.user.id" name="email"
                            data-ng-model="usrCtrl.user.email" class="form-control"
                            placeholder="{{ trans('user::adminuser.email_placeholder') }}"
                            value="{{ old('email') }}" />
                        <input type="text" data-ng-if="usrCtrl.user.id" readonly name="email"
                            data-ng-model="usrCtrl.user.email" class="form-control"
                            placeholder="{{ trans('user::adminuser.email_placeholder') }}"
                            value="{{ old('email') }}" />
                    </div>
                ...
                || (event.keyCode==46) )"
                            data-validation-name="Phone Number" name="phone" maxlength="15" class="form-control"
                            data-validation-name="Phone Number" data-ng-model="usrCtrl.user.phone"
                ...
                <div class="form-group" data-ng-class="{'has-error': errors.user_group_id.has}">
                    ...
                    <div class="form-input">
                        <select class="form-control mb10 select2_custom_ddl" data-jquery="select2_custom_ddl"
                            data-validation-name="{{ trans('user::adminuser.user_group') }}" name="user_group_id"
                            myValue="usrCtrl.user.user_group_id"
                            myPlaceholder="{{ trans('user::adminuser.user_group') }}"
                            data-ng-model="usrCtrl.user.user_group_id"
                ...
                <div class="form-group" data-ng-class="{'has-error': errors.gender.has}">
                    ...
                    <div class="form-input">
                        <select class="form-control" data-jquery="select2_custom_ddl" name="gender"
                            myPlaceholder="{{ trans('user::adminuser.select_gender') }}"
                            data-ng-model="usrCtrl.user.gender" myValue="usrCtrl.user.gender">
                            <option value=""></option>
            ...
            <div class="bottom-button text-right flexbox align-items-center">
                <input type="button" value="{{ trans('base::general.cancel') }}"
                    data-ng-click="usrCtrl.closeUserEdit()" name="cancel" class="save" />
