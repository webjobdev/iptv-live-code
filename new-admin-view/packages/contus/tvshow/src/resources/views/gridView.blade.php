    ...
    <div id="latest_video">
        ...
        <div class="table_responsive" id="fixTable_parent">
            <table class="table tablesaw" id="fixTable" data-tablesaw-mode="columntoggle"
                ...
                <thead>
                    <tr>
                        @include('audio::admin.common.bulkActionLayout', ['access_type' => 'tv_show'])
                ...
                <tbody>
                    <tr class="search_text">
                        ...
                        <td class="search_product">
                            <input type="text" class="form-control" data-ng-model="searchRecords.title"
                                data-boot-tooltip="true" title="Enter Tv Show Title" placeholder="Enter Tv Show Name">
                        </td>
                        <td></td>
                        <td></td>
                        <td></td>
                        ...
                        <td class="search_product">
                            <select class="select2_custom_ddl" minimumresults="-1" data-jquery="select2_custom_ddl"
                                ...
                                data-ng-model="searchRecords.is_parental" data-boot-tooltip="true"
                                title="{{__('base::general.select_status')}}">
                                <option value="all">{{__('base::general.all')}}</option>
                                <option value='1'>Pin Locked</option>
                                <option value='0'>Not Pin Locked</option>
                            </select>
                        </td>
                        <td></td>
                        <td class="search_product">
                    ...
                    <tr data-ng-if="showRecords" data-ng-repeat="record in records track by $index"
                        ...
                        <td>
                            ...
                            <div class="ckbox ckbox-default">
                                <input type="checkbox" class="checkbox" id="roles_@{{record.id}}"
                                    ng-click="tvsCtrl.selectRecord($event, record.id)" value="@{{record.id}}"
                        ...
                        <!-- season -->
                        <td>
                            @{{ record.total_season || '0' }}
                        </td>

                        <!-- episode -->
                        <td>
                            @{{ record.total_episode || '0' }}
                        </td>
                        ...
                        <td>
                            <div data-ng-if="record.is_parental != 1">
                                <svg fill="#3cff00ff" xmlns="http://www.w3.org/2000/svg" width="25px" height="25px"
                                    ...
                                    <g>
                                        <path
                                            d="M11,19h4c0.6,0,1-0.3,1-0.9V18c0-5.7,4.9-10.4,10.7-10C32,8.4,36,13,36,18.4v-0.3c0,0.6,0.4,0.9,1,0.9h4 c0.6,0,1-0.3,1-0.9V18c0-9.1-7.6-16.4-16.8-16c-8.5,0.4-15,7.6-15.2,16.1C10.1,18.6,10.5,19,11,19z" />
                                        <path d="M10,18.1v0.4C10,18.4,10,18.3,10,18.1L10,18.1z" />
                                        <path
                                            d="M46,27c0-2.2-1.8-4-4-4H10c-2.2,0-4,1.8-4,4v19c0,2.2,1.8,4,4,4h32c2.2,0,4-1.8,4-4V27z M30.6,42.7 c0.2,0.6-0.3,1.3-1,1.3h-7.3c-0.7,0-1.1-0.6-1-1.3l1.8-6c-1.5-1-2.4-2.8-2.1-4.8c0.4-1.9,1.9-3.4,3.9-3.8c3.2-0.6,6,1.7,6,4.7 c0,1.6-0.8,3.1-2.1,3.9L30.6,42.7z" />
                                    </g>
                                </svg>
                            </div>
                            <div data-ng-if="record.is_parental == 1">
                        ...
                        <!-- status -->
                        <td>
                            <span class="status-active" ng-if="record.is_active == 1">
                                Active
                            </span>
                        ...
                        <!-- updated date -->
                        <td>
                            @{{ record.updated_at | date:'d/M/y H:m:s' }}
                        </td>
                        ...
                        <!-- unpublish date -->
                        <td>
                            @{{ record.expire_scheduled_time ? (record.expire_scheduled_time | date:'dd/MM/yy HH:mm:ss')
                            : '-' }}
                        </td>
                        ...
                        <td class="table-action">
                            <div class="flexbox align-items-center justify-center">
                                ...
                                <div class="form-group row" style="margin-bottom: 0px; margin-right: 5px;">
                                    <label class="switch">
                                        <input type="checkbox" ng-checked="record.is_active == 1"
                                            ng-click="tvsCtrl.togglePublishNow(record, record.id)">
                                ...
                                <div ng-if="checkAccess('tv_shows.edit')" class="column edit_table_icon tooltip-parent">
                                    <a class="table_action"
                                        href="{{url('admin/tvshow/edit-tv-show')}}/@{{ encodeId(record.id) }} ">
                                ...
                                <!-- <div class="tooltip-parent" data-ng-if="checkAccess('tv_show')">
                                    <span class="delete_table_icon"
                                        ...
                                        data-ng-click="tvsCtrl.deleteSingleRecordVideos(record.id,record.title)">
                                ...
                                <div class="tooltip-parent" data-ng-if="checkAccess('tv_shows.delete')">
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form name="collectionForm" method="POST" data-base-validator="" data-ng-submit="tvsCtrl.save($event)"
                    enctype="multipart/form-data">
                    {!! csrf_field() !!}
                    <div class="collection_model">
                        <div class="collection_listing">
                            <div class="rdio rdio-primary collection_listing_input"
                                data-ng-repeat="(key, value) in tvsCtrl.allCollections">
                                <input type="radio" name="radio" data-ng-value="@{{key}}"
                                    data-ng-model="tvsCtrl.collection.id" data-ng-change="tvsCtrl.createCollection(key)"
                                    id="radioPrimary_@{{key}}">
...
<div id="videoPresetsModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="preset_wrap" data-ng-repeat="preset in tvsCtrl.commonVideoPresets track by $index">@{{
                    $index+1 }}. @{{ preset }}</div>
            </div>
            <div class="modal-footer">
