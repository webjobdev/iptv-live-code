...
@section('content')
    @include('tvshow::common.subMenu', ['template' => 'edit_tvshow'])

    <div class="form-page" data-base-validator data-ng-controller="TvShowUploadController as tvsGridCtrl"
        id="video-detail-start" data-ng-init="tvsGridCtrl.fetchData('{{$id}}')">
        ...
        <div id="video_accordion_wrapper">
            <div class="form-page">
                @include('tvshow::TvShowForm')

                @include('tvshow::common.popup', ['control' => 'tvsGridCtrl'])
            </div>
        </div>
    </div>
@endsection
...
@section('scripts')
    ...
    <script src="{{asset('adminview/assets/js/tvshow/TvshowUpload.js')}}"></script>
