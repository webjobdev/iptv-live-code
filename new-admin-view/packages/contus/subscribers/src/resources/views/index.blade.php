...
@section('content')
<div id="home" data-ng-controller="SubscriberController as subCtrl">
    <div class="" id="dashboard-page">
        <div class="page-heading flexbox align-items-center flex-wrap">
            <h4>{{__('subscribers::index.subscribers')}}</h4>
            <div class="right-side flexbox align-items-center">
                <a data-ng-if="checkAccess('subscribers.create')" data-ng-click="subCtrl.addOrganization($event)" href="javascript:void(0)" class="button button-blue sidepanel-open">
        ...
        <!-- <form class="form-inline serch-btn" id="searchForm" method="GET">
            {!! csrf_field() !!}
            <input class="form-control mr-2" type="search" name="organization_name" placeholder="Search here..." aria-label="Search" id="searchInput">&nbsp;
            <button class="btn btn-info glyphicon glyphicon-search" type="submit" style="background-color: #00ACCD;"></button>
        </form> -->
        <br>
        <div class="contentpanel product order_list">
            ...
            <div data-grid-view data-rows-per-page="10"
                data-route-name="subscribers/get-all"
                data-template-route="admin/subscribers" data-count="false"></div>
        </div>
    </div>
</div>
@endsection
...
<script src="{{asset('adminview/assets/js/subscribers/index.js')}}"></script>
