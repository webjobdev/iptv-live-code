<!-- note table code -->
<div id="notecard">
    <div id="table_loader" class="table_loader_container" data-ng-show="gridLoadingBar">
        <div class="table_loader">
            <div class="loader"></div>
        </div>
    </div>
    <div class="table_responsive">
        <table class="table subscription-plan-grid" id="fixTable" data-ng-class="{'no-records': noRecords}">
            <thead>
                <tr>
                    @include('audio::admin.common.bulkActionLayout', ['access_type' => 'subscribers'])
                    <th data-ng-repeat="field in heading"
                        ng-class="{'centre': field.name == 'No. of Videos' || field.name == 'order'}">
                        @{{field.name}}
                        <span data-ng-if="field.sort==true" id="" class="th-inner sortable both"
                            data-ng-class="{showGridArrow:field.sort}"
                            data-ng-click="fieldOrder($event,field.value)"></span>
                        <span data-ng-if="field.sort==false" data-ng-class="{showGridArrow:field.sort}"></span>
                    </th>
                </tr>
            </thead>

            <tbody>
                <tr class="search_text">
                    <td></td>
                    <td></td>
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.profile_name"
                            placeholder="Profile Name" data-boot-tooltip="true" data-toggle="tooltip"
                            data-original-title="Profile Name">
                    </td>
                    <td></td>
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.note_type"
                            placeholder="Note Type" data-boot-tooltip="true" data-toggle="tooltip"
                            data-original-title="Note Type">
                    </td>
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.sub_note_type"
                            placeholder="Sub Note Type" data-boot-tooltip="true" data-toggle="tooltip"
                            data-original-title="Sub Note Type">
                    </td>
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.subject"
                            placeholder="Subject" data-boot-tooltip="true" data-toggle="tooltip"
                            data-original-title="Subject">
                    </td>
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.description"
                            placeholder="Description" data-boot-tooltip="true" data-toggle="tooltip"
                            data-original-title="Description">
                    </td>
                    <td></td>
                    <td></td>
                </tr>
                <tr data-ng-if="noRecords">
                    <td colspan="8" colspan="@{{heading.length + 1}}" class="no-data center">
                        {{trans('base::general.not_found')}}
                    </td>
                </tr>
                <tr data-ng-if="showRecords"
                    data-ng-repeat="record in records | filter:{ subscriber_id: subscriberIdFromUrl } track by $index"
                    data-ng-show="showRecords" class="list-repeat" data-intialize-sidebar="">
                    <td>
                        <div class="ckbox ckbox-default">
                            <input type="checkbox" class="checkbox" id="roles_@{{record.id}}"
                                ng-click="selectRecord($event, record.id)" value="@{{record.id}}"
                                name="selectedCheckbox[]">
                            <label for="roles_@{{record.id}}"></label>
                        </div>
                    </td>

                    <td class="serial_number">@{{ ((currentPage - 1) * rowsPerPage) + $index + 1 }}</td>
                    <td class="">@{{ record.created_at | date:'dd-MM-yyyy'}}</td>
                    <td class="">@{{ record.subscriber_details.first_name + ' ' + record.subscriber_details.last_name }}
                    </td>
                    <td class="">@{{ record.note_type }}</td>
                    <td class="">@{{ record.sub_note_type || '-' }}</td>
                    <td class="">@{{ record.subject || '-'}}</td>
                    <td class="">@{{ record.description}}</td>
                    <td class="">@{{ record.updates || 'not updated'}}</td>

                    <td class="table-action">
                        <!-- view button -->
                        <div data-ng-if="checkAccess('notes.view')" class="column edit_table_icon tooltip-parent">
                            <button data-ng-click="snCtrl.viewrecord(record)" data-toggle="modal"
                                data-target="#flipFlop">
                                <i class="fa fa-eye" aria-hidden="true"></i>
                            </button>
                        </div>



                        <!-- edit button -->
                        <div data-ng-if="checkAccess('notes.edit')" class="column edit_table_icon tooltip-parent">
                            <button data-ng-click="snCtrl.fetchnote(record)" data-toggle="modal"
                                data-target="#flipFlop">
                                <svg viewBox="0 0 12 11" x="0px" y="0px" width="12px" height="11px">
                                    <g>
                                        <path
                                            d="M 10.7871 0.7184 C 9.8401 -0.2303 8.3003 -0.2303 7.3535 0.7184 L 1.0581 7.0082 C 1.0085 7.0574 0.9783 7.1206 0.969 7.1885 L 0.5024 10.6415 C 0.4885 10.74 0.5237 10.8383 0.5916 10.9062 C 0.6504 10.9648 0.7322 10.9998 0.8145 10.9998 C 0.8284 10.9998 0.8423 10.9998 0.8564 10.9976 L 2.9377 10.7165 C 3.1111 10.693 3.2332 10.5337 3.2097 10.3604 C 3.186 10.1871 3.0269 10.0652 2.8535 10.0886 L 1.1846 10.3135 L 1.5103 7.9054 L 4.0464 10.4401 C 4.105 10.4985 4.1873 10.5337 4.269 10.5337 C 4.3511 10.5337 4.4333 10.5009 4.4919 10.4401 L 10.7871 4.1502 C 11.2463 3.6911 11.4998 3.0821 11.4998 2.4332 C 11.4998 1.7842 11.2463 1.1752 10.7871 0.7184 ZM 7.4753 1.4914 L 8.5325 2.5479 L 2.7876 8.2896 L 1.7307 7.2331 L 7.4753 1.4914 ZM 4.2712 9.77 L 3.2378 8.737 L 8.9822 2.9954 L 10.0159 4.0284 L 4.2712 9.77 ZM 10.4568 3.5763 L 7.9277 1.0486 C 8.2488 0.784 8.6497 0.6387 9.0715 0.6387 C 9.5518 0.6387 10.0022 0.8261 10.3418 1.1634 C 10.6816 1.5008 10.8669 1.9529 10.8669 2.4332 C 10.8669 2.8572 10.7214 3.2554 10.4568 3.5763 Z"
                                            fill="#454545"></path>
                                    </g>
                                </svg>
                            </button>
                            <span class="tooltip_title">{{trans('base::general.edit')}}</span>
                        </div>



                        <!-- delete button -->
                        <div class="tooltip-parent" data-ng-if="checkAccess('notes.edit')">
                            <span ng-mouseover="getTooltip($event)" data-toggle="modal" data-target="#deleteModal"
                                ng-click="deleteSingleRecord(record.id)" class="tooltips delete_table_icon"
                                data-boot-tooltip="true" data-original-title="">
                                <svg viewBox="0 0 11 12" x="0px" y="0px" width="11px" height="12px">
                                    <g data-original-title="" title="">
                                        <path
                                            d="M 10.4998 3.513 L 9.6099 3.513 L 8.9153 11.6068 C 8.8962 11.8292 8.7144 11.9998 8.4966 11.9998 L 2.4885 11.9998 C 2.2708 11.9998 2.0889 11.8293 2.0701 11.6069 L 1.3752 3.513 L 0.4995 3.513 L 0.4995 2.6513 L 3.4268 2.6513 L 3.4268 1.7179 C 3.4268 1.322 3.741 0.9999 4.1272 0.9999 L 6.8721 0.9999 C 7.2583 0.9999 7.5725 1.322 7.5725 1.7179 L 7.5725 2.6513 L 10.4998 2.6513 L 10.4998 3.513 ZM 6.7322 1.8615 L 4.2668 1.8615 L 4.2668 2.6513 L 6.7322 2.6513 L 6.7322 1.8615 ZM 2.2188 3.513 L 2.873 11.1383 L 8.1121 11.1383 L 8.7661 3.513 L 2.2188 3.513 ZM 6.5222 9.8588 L 6.7043 4.7609 L 7.5442 4.7924 L 7.3621 9.8902 L 6.5222 9.8588 ZM 5.0796 4.7767 L 5.9199 4.7767 L 5.9199 9.8746 L 5.0796 9.8746 L 5.0796 4.7767 ZM 3.4551 4.7923 L 4.2949 4.7608 L 4.4771 9.8586 L 3.6372 9.8902 L 3.4551 4.7923 Z"
                                            fill="#454545"></path>
                                    </g>
                                </svg>
                                <span class="tooltip_title">{{trans('base::general.delete')}}</span>
                            </span>
                        </div>


                    </td>
                </tr>
            </tbody>
        </table>

    </div>
    <!-- @include('audio::admin.common.singleRecordDeleteModal')
@include('audio::admin.common.singleRecordStatusUpdateModal') -->
    @include('base::layouts.pagination')
</div>


<!-- ==============================***********************************============================== -->
<!-- ==============================***********************************============================== -->


<div class="modal fade" id="flipFlop" tabindex="-1" role="dialog" aria-labelledby="modalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">

            <!-- Modal Header -->
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
                <h4 class="modal-title" id="modalLabel" data-ng-if="!snCtrl.note.id">Add Note</h4>
                <h5 data-ng-if="snCtrl.note.id">Edit Subscriber Note</h5>
            </div>

            <form method="POST" enctype="multipart/form-data" data-base-validator
                data-ng-submit="snCtrl.save($event, snCtrl.note.id)">
                {!! csrf_field() !!}

                <input type="hidden" id="subscriber-id" name="subscriber-id"
                    value="{{ request()->query('subscriber-id') }}">
                <input type="hidden" id="id" name="id" ng-model="snCtrl.note.id">


                <div class="modal-body" style="padding: 20px;">

                    <div class="panel panel-default" style="border-radius: 6px; box-shadow: rgba(0, 0, 0, 0.16) 0px 10px 36px 0px, rgba(0, 0, 0, 0.06) 0px 0px 0px 1px;">
                        <div class="panel-body">
                            <!-- note type -->
                            <div class="form-group row">
                                <label class="col-sm-3 control-label"
                                    style="font-weight: 600; color: #555; margin-top: 7px;">
                                    <!-- {{trans('note::adminnote.security_type')}} -->
                                    Note Type <span class="required">*</span></label>
                                <div class="form-input">
                                    <select class="form-control" data-jquery="select2_custom_ddl" name="note_type"
                                        ng-disabled="snCtrl.isViewOnly" data-ng-change="snCtrl.updateCardPattern()"
                                        myPlaceholder="Choose note type" data-ng-model="snCtrl.note.note_type"
                                        myValue="snCtrl.note.note_type">
                                        <option value=""></option>
                                        <option value="technical_issue">Technical issue</option>
                                        <option value="note_issue">note issue</option>
                                        <option value="consultation">Consultation</option>
                                        <option value="complaint">Complaint</option>
                                        <option value="suggestion">Suggestion</option>
                                        <option value="sale">Sale</option>
                                        <option value="other">Other</option>
                                    </select>
                                </div>
                                <p class="error-msg" data-ng-show="errors.gender.has">@{{ errors.gender.message }}</p>
                            </div>

                            <!-- issue with -->
                            <div class="form-group row" data-ng-if="snCtrl.note.note_type == 'technical_issue'">
                                <label class="col-sm-3 control-label"
                                    style="font-weight: 600; color: #555; margin-top: 7px;">
                                    <!-- {{trans('note::adminnote.security_type')}} -->
                                    Issue with <span class="required">*</span></label>
                                <div class="form-input">
                                    <select class="form-control" data-jquery="select2_custom_ddl" name="security_type"
                                        data-ng-change="snCtrl.updateCardPattern()" ng-disabled="snCtrl.isViewOnly"
                                        myPlaceholder="Choose issue with" data-ng-model="snCtrl.note.sub_note_type"
                                        myValue="snCtrl.note.sub_note_type">
                                        <option value=""></option>
                                        <option value="device">Device</option>
                                        <option value="clinet_app">Client App</option>
                                        <option value="others">Others</option>
                                    </select>
                                </div>
                                <p class="error-msg" data-ng-show="errors.gender.has">@{{ errors.gender.message }}</p>
                            </div>

                            <!-- note subject -->
                            <div class="form-group row"
                                data-ng-if="snCtrl.note.note_type === 'consultation' || snCtrl.note.note_type === 'suggestion' || snCtrl.note.note_type === 'other'">
                                <label class="col-sm-3 control-label"
                                    style="font-weight: 600; color: #555; margin-top: 7px;">Note Subject <span
                                        class="required">*</span></label>
                                <div class="form-input col-sm-6" style="padding-right: 10px;">
                                    <input type="text" name="note_subject" data-unique="@{{snCtrl.uniqueRoute}}"
                                        data-ng-model="snCtrl.note.subject" ng-disabled="snCtrl.isViewOnly"
                                        class="form-control" placeholder="Enter subject" />
                                </div>
                                <p class="error-msg col-sm-12">@{{ errors.first_name.message }}</p>
                            </div>

                            <!-- complaint type -->
                            <div class="form-group row" data-ng-if="snCtrl.note.note_type == 'complaint'">
                                <label class="col-sm-4 control-label"
                                    style="font-weight: 600; color: #555; margin-top: 7px;">
                                    <!-- {{trans('note::adminnote.security_type')}} -->
                                    Complaint about <span class="required">*</span></label>
                                <div class="form-input">
                                    <select class="form-control" data-jquery="select2_custom_ddl" name="security_type"
                                        data-ng-change="snCtrl.updateCardPattern()" ng-disabled="snCtrl.isViewOnly"
                                        myPlaceholder="Choose complaint about" data-ng-model="snCtrl.note.sub_note_type"
                                        myValue="snCtrl.note.sub_note_type">
                                        <option value=""></option>
                                        <option value="device">Employee</option>
                                        <option value="clinet_app">Client App</option>
                                        <option value="advertising">Advertising</option>
                                        <option value="content">Content</option>
                                        <option value="others">Others</option>
                                    </select>
                                </div>
                                <p class="error-msg" data-ng-show="errors.gender.has">@{{ errors.gender.message }}</p>
                            </div>

                            <!-- description -->
                            <div class="form-group">
                                <label style="font-weight: 600; color: #555; margin-top: 7px;">Note Description <span
                                        class="required">*</span></label>
                                <!-- <textarea name="description" class="form-input" id="editor" data-ng-model="snCtrl.note.description" placeholder="Description"></textarea> -->
                                <input name="htmlcode" id="inp_htmlcode" type="hidden" />
                                <div id="div_editor1" class="richtexteditor" data-ng-model="description"
                                    ng-disabled="snCtrl.isViewOnly"
                                    style="width: 100%; max-width:820px; margin:0 auto;"></div>
                            </div>

                        </div>
                    </div>
                </div>

                <!-- Modal Footer -->
                <div class="modal-footer bottom-button text-right flexbox align-items-center">
                    <input type="button" value="{{ trans('base::general.cancel') }}" data-dismiss="modal"
                        data-ng-click="snCtrl.closeNoteForm()" name="cancel" class="save button button-gray">

                    <input type="submit" value="{{ trans('base::general.submit') }}" style="margin-left: 10px;"
                        ng-disabled="snCtrl.isViewOnly" name="submit" class="button button-blue">
                </div>
            </form>

        </div>
    </div>
</div>


<!-- ==============================***********************************============================== -->
<!-- ==============================***********************************============================== -->