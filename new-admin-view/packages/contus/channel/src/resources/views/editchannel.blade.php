...
@section('content')
    @include('channel::common.subMenu', ['template' => 'edit_channel'])

    <div class="form-page" data-base-validator data-ng-controller="channelUploadController as channelGridCtrl"
        id="video-detail-start" data-ng-init="channelGridCtrl.fetchData('{{$id}}')">
        ...
        <div id="video_accordion_wrapper">
            <div class="form-page">
                @include('channel::channelform')

                @include('channel::common.popup', ['control' => 'channelGridCtrl'])
            </div>
        </div>
    </div>
@endsection
...
@section('scripts')
    ...
    <script src="{{asset('adminview/assets/js/channel/chanelUpload.js')}}"></script>
