...
namespace Contus\Playlist\Helpers;
...
class UploadHandler{
    ...
    public function combineChunks($uploadDirectory, $name = null)
    {
        ...
        chmod($uploadDirectory.DIRECTORY_SEPARATOR.$uuid, 0777);        
        chmod($uploadDirectory.DIRECTORY_SEPARATOR.$uuid.DIRECTORY_SEPARATOR.$name, 0777);
    ...
    public function handleUpload($uploadDirectory, $name = null)
    {
        ...
        $postMaxSize = filter_var(ini_get('post_max_size'), FILTER_SANITIZE_NUMBER_INT);
        $uploadMaxFilesize = filter_var(ini_get('upload_max_filesize'), FILTER_SANITIZE_NUMBER_INT);
        if ($this->toBytes($postMaxSize) < $this->sizeLimit ||
            $this->toBytes($uploadMaxFilesize) < $this->sizeLimit) {
