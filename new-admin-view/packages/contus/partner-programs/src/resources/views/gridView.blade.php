<div id="announcment">
    ...
    <div class="table_responsive">
        <table class="table subscription-plan-grid" id="fixTable" data-ng-class="{'no-records': noRecords}">
            <thead>
                <tr>
                    @include('audio::admin.common.bulkActionLayout', ['access_type' => 'partner_programs'])
                    <th data-ng-repeat="field in heading">
                        @{{ field.name }}
                        <span data-ng-if="field.sort==true" class="th-inner sortable both"
            ...
            <tbody>
                <tr class="search_text">
                    ...
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.program_name"
                            data-boot-tooltip="true" data-toggle="tooltip" data-original-title="Enter Program Name" 
                            placeholder="Enter Program Name">
                    </td>
                    <td class="search_product">
                        <input type="text" class="form-control" id="created-by-search" data-ng-model="searchRecords.created_by"
                            data-boot-tooltip="true" data-toggle="tooltip" data-original-title="Enter Users Name" 
                ...
                <tr data-ng-if="noRecords">
                    <td colspan="@{{ heading.length + 1 }}" class="no-data center">
                ...
                <tr data-ng-if="showRecords" data-ng-repeat="record in records track by $index"
                    ...
                    <td style="display: flex; gap:10px;">
                        <img src="@{{ record.partner_app_logo_url }}" alt="" height="40px" width="40px" 
                            class="partner_app_logo" style="border: 1px solid white; border-radius: 20px"> 
                        @{{ record.partner_provider }}
                    </td>

                    <td>@{{ record.program_name }}</td>
                    <td>@{{ record.user[0].name }}</td>
                    <td>@{{ record.formatted_updated_at }}</td>

                    <td class="table-action">
                        <div class="flexbox align-items-center justify-center">
                            <div class="column edit_table_icon tooltip-parent">
                                <button data-ng-click="programCtrl.editPartnerProgram(record.id)">
                            ...
                            <div class="tooltip-parent">
                                <span ng-mouseover="getTooltip($event)" data-toggle="modal" data-target="#deleteModal"
                                    ...
                                    <svg viewBox="0 0 11 12" width="11px" height="12px">
