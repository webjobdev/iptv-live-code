    ...
    */
    ...
    public function editVideo($id) {
        if (!empty ($id)) {
            ...
            if($this->request->has('newVideoUUID') && $this->request->newVideoUUID != '') {
        ...
        if (!empty ($id)) {
            ...
            if ($video->is_live) {
                $this->setRules([
                    ...
                    // 'is_featured' => StringLiterals::REQUIREDINTEGER,
                ]);
            } else {
                $this->setRules([
                    StringLiterals::TITLE => StringLiterals::REQUIRED,
    ...
    public function getGridHeadings()
    {
        ...
        if ($checkLive) {
            return [StringLiterals::GRIDHEADING => [['name' => trans('video::videos.title'), StringLiterals::VALUE => StringLiterals::TITLE, 'sort' => true], 
            ['name' => trans('video::videos.status'), StringLiterals::VALUE => StringLiterals::ISACTIVE, 'sort' => false], ['name' => 'Type', StringLiterals::VALUE => StringLiterals::ISACTIVE, 'sort' => false],
    ...
    {    
        ...
        $title = $is_active = $type = $video_type = $category = null;
    ...
    {   
        $videoIdArray = explode(',', base64_decode($id));
        return $this->video->with('categories', 'tags', 'collections', 'seasons', 'videoTranslation', 'ads', 'cast', 'videoAudioTracks')->whereIn('id', $videoIdArray)->where(StringLiterals::IS_ARCHIVED, 0)->get();
    }
    ...
    {     
      ...
      if($checkVideoExist)
      {
        ...
        $checkVideoExist->updator_id=$this->authUser->id;
        ...
        $checkVideoExist->description=$this->request->description;
        $checkVideoExist->keyword=$this->request->keyword;
        $checkVideoExist->creator_id=$this->authUser->id;
        $checkVideoExist->save();
