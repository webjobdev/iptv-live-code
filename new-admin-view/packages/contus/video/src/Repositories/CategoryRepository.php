...
class CategoryRepository extends BaseRepository implements ICategoryRepository {
    ...
    public function getAllCategoryList() {
        ...
        if (sizeof ( $categories ) > 0) {
            foreach ( $categories as $category ) {
                $categoryList = $categoryList . '<li id="category_id_' . $category->id . '"><div class="rdio"><input type="radio" name="parent_id" data-ng-model="catgridCtrl.category.parent_id" value=' . $category->id . '> <label> <svg viewBox="0 0 201 156" version="1.1" x="0px" y="0px" width="13px" height="10px"> <g> <path d="M 1 156 L 1 1 L 64 1 L 91 46 L 201 46 L 201 156 L 1 156 ZM 91 16 L 201 16 L 201 31 L 100 31 L 91 16 Z"  fill="#014ECE"/> </g> </svg> <span>' . $category->title . '</span></label></div>';
    ...
    public function getAllCategoryInfo($slug = '') {
        $catInfo = $this->_category->selectRaw('id, title')->where ( 'parent_id', 0 )->where ( 'is_active', 1 )->with ( 'child_category.child_category' )->get ()->toArray ();
        return $catInfo;
    }
    ...
    protected function updateGridQuery($builder) {
        ...
        $builder->selectRaw('categories.*, categories.id as formatted_created_date')->where ( 'is_deletable', 1 );
        return $builder;
    }
    ...
    public function getCategoryFromWebSeries($id) {
        return $this->_category->where('video_webseries_detail_id',$id)->first();
    }
