...
var CategoryGridController = ['$scope', '$rootScope','requestFactory', '$window', '$sce', '$timeout', '$compile', '$interval', function (scope, rootScope, requestFactory, $window, $sce, $timeout, $compile, $interval) {
    ...
    this.deleteCategoryImage = function () {
        requestFactory.toggleLoader();
        requestFactory.post(requestFactory.getUrl('radiocategory/delete-category-image/' + this.category.id), this.category, function (response) {
            requestFactory.toggleLoader();
    ...
    this.fetchInfo = function () {
        requestFactory.get(requestFactory.getUrl('radiocategory/info'), this.defineProperties, function (response) { 
    ...
    this.addCategory = function ($event) {
        ...
        self.category = {};
        self.categoryFull = {};
        this.categoriesUniqueRoute = requestFactory.getUrl('radiocategory/categories-unique');
        this.category = {};
        this.categoryFull = {};
    ...
    this.editCategory = function (records) {
        ...
        this.categoriesUniqueRoute = requestFactory.getUrl('radiocategory/categories-unique/' + records.id);
    ...
    this.categorySave = function ($event, id) {
        ...
        if (baseValidator.validateAngularForm($event.target, scope)) {
            ...
            this.category.is_category = 1;
            if (id) {
                requestFactory.post(requestFactory.getUrl('radiocategory/edit/' + id), this.category, function (response) {
                    requestFactory.setToaster('success', response.message);
                    requestFactory.getToaster();
                    scope.getRecords(true);
                ...
                requestFactory.post(requestFactory.getUrl('radiocategory/add'), this.category, function (response) {
                    requestFactory.setToaster('success', response.message);
                    requestFactory.getToaster();
                    scope.getRecords(true);
                    this.closeCategoryEdit();
    ...
    this.categoryTranslateSave = function (event, id) {
        ...
        requestFactory.post(requestFactory.getUrl('radiocategory/addLanguage/' + id), this.categoryTranslation, function (response) {
            requestFactory.setToaster('success', response.message);
            requestFactory.getToaster();
  ...
  this.checkMasterCheckbox = function () {
    ...
    this.deleteBulkRecords = function (id, status) {
        ...
        scope.deleteParams = '';
        scope.deleteRequest = requestFactory.post(requestFactory.getUrl('radiocategory/action'), angular.extend({}, {
            selectedCheckbox: id,
            status: status
    ...
    this.activateOrDeactivateRecords = function (id, is_status) {
        ...
        if (is_status == 1) {
            scope.deleteRequest = requestFactory.post(requestFactory.getUrl('radiocategory/bulk-update-status'), angular.extend({}, {
                selectedCheckbox: id,
                isStatus: 'activate'
            }, scope.requestParams), function (data) {
                requestFactory.setToaster('success', data.message);
    ...
    scope.$on('afterGetRecords', function (e, data) {
        ...
        requestFactory.get(requestFactory.getUrl('radiocategory/updated-details'), function (data) {
