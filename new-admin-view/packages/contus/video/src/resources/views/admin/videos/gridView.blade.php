                ...
                <thead>
                    <tr>
                       ...
                       <!-- Check Menu Flag  -->
                           ...
                           @if ($page === 'vod')
                            <div class="dropdown bulkaction" style="float: left; right: 20px;" data-ng-show="vgridCtrl.selectedRecords != 0 && checkAccess('videos_all_write')"
                            ...
                            @if ($page === 'live')
                            <div class="dropdown bulkaction" style="float: left; right: 20px;" data-ng-show="vgridCtrl.selectedRecords != 0 && checkAccess('live_videos_all_write')"
                                data-original-title="Select video in the grid to perform a bulk action">
                                <button class="btn btn-warning dropdown-toggle" type="button" data-toggle="dropdown">
                                    {{__('video::videos.bulk_action')}}
                                    <span class="caret"></span>
                ...
                <tbody>
                    <tr class="search_text">
                        ...
                        <td class="vod-grid-categ">
                            @if ($page === 'vod')
                            <select multiple data-jquery="select2_custom_ddl" myValue="selectedVideo.category"
                            ...
                            name="category"
                            class="select2_custom_ddl" style="width:100px"
                            data-ng-model="searchRecords.category" data-boot-tooltip="true" title="{{__('video::videos.enter_video_category')}}" data-ng-change="search()">
                                <option value=""></option>
                            </select>
                            @else
                        ...
                        <td class="center" style="display: none;">
                            <svg style="display:none" viewBox="0 0 16 16" x="0px" y="0px" width="16px" height="16px">
                                <g>
                    ...
                    <tr data-ng-if="showRecords" data-ng-repeat="record in records track by $index" data-ng-show="showRecords"
                        ...
                        <td>
                            <div class="product_img flexbox align-items-center" >
                                <a ng-if="record.is_live==0" href="{{url('admin/videos/view-details-video')}}/@{{record.id}}" class="table-image-text flexbox align-items-center">
                                    <div class="image" bg-image="@{{record.thumbnail_image}}" on-error-src="{{asset('adminview/assets/images/default_image.png')}}">
                                    </div>
                                ...
                                <a ng-if="record.is_live==1" href="{{url('admin/livevideos/view-details-video')}}/@{{record.id}}" class="table-image-text flexbox align-items-center">
                                    <div class="image" bg-image="@{{record.thumbnail_image}}" on-error-src="{{asset('adminview/assets/images/default_image.png')}}"></div>
                                    <div class="product_description tooltip-parent "  data-ng-class="{'failed': record.job_status == 'Error'||record.job_status == 'Canceled'||record.job_status == 'Error Recording'}">
                                        <p  class="img_description" >@{{ record.title}}
                                            <span class="tooltip_title">@{{record.title}}</span>
                                        </p>
                        ...
                        <td class="upload-status" ng-if="record.is_live==0">
                            <div class="status-inprogress" ng-if="record.job_status == 'Progressing'">
                                <span class="status-transcoding">{{__('video::videos.video_upload_status.transcoding')}} @{{record.upload_percentage}}%</span>
                            </div>


                            <span class="status-transcoding" ng-if="(record.job_status == 'Video Uploaded' || record.job_status == 'Uploading' || record.job_status == 'Uploaded' || record.job_status == 'Convert to MP4' || record.job_status == 'Submitted') && record.is_live==0"> {{__('video::videos.video_upload_status.transcoding')}} ...</span>
                            ...
                            <button ng-if="record.username !== '' && record.is_live==1 && record.username != 'wowza'"  data-toggle="modal" data-target="#livestreamStopModal" ng-click="stoplivestream(record)" ng-show="record.liveStatus==='streaming'" class="status-label stop-btn" title="{{__('video::videos.video_upload_status.stop_livestreaming')}}" data-boot-tooltip="true" >{{__('video::videos.stop_live')}}</button>
                            <span class="type-label type_live" ng-show="record.liveStatus==='Complete'">Complete</span>
                            <span class="type-label type_live" ng-show="record.liveStatus==='Uploaded'">Uploaded</span>
                        </td>
                        ...
                        <td ng-if="record.is_live !== 1" class="vod-grid-categ">
                        ...
                        <!-- <td ng-if="record.is_live !== 1"class="center">@{{ record.price| number:2 }}</td> -->
                        <!-- <td ng-if="record.is_live == 1">@{{ record.scheduledStartTime }}</td> -->

                        <td ng-if="record.is_live !== 1" class="center">@{{ record.view_count }}</td>
                        ...
                        <td class="table-action">
                            <div class="flexbox align-items-center justify-center">
                                ...
                                <div  ng-if="record.is_live != 1 && checkAccess('videos_all_write')"  class="column edit_table_icon tooltip-parent">
