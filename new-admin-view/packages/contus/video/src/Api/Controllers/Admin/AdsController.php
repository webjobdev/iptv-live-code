...
use Contus\Video\Repositories\AdsRepository;
...
class AdsController extends ApiController {
  ...
  public function postAdd() {
    ...
    if ($addCategory) {
      $isCategoryAdd = true;
      $this->request->session ()->flash ( StringLiterals::SUCCESS, trans ( 'video::ads.added' ) );
    }
    return ($isCategoryAdd) ? $this->getSuccessJsonResponse ( [
        StringLiterals::STATUS => 'success',
        StringLiterals::MESSAGE => trans ( 'video::ads.success.added' )
    ] ) : $this->getErrorJsonResponse ( [
        [
            StringLiterals::STATUS => 'error',
            StringLiterals::MESSAGE => trans ( 'video::ads.error.added' )
        ]
    ] );
    }
  }
  ...
  public function postEdit($id) {
    ...
    if ($editCategory) {
      $isCategoryEdit = true;
      $this->request->session ()->flash ( StringLiterals::SUCCESS, trans ( 'video::ads.updated' ) );
    }
    return ($isCategoryEdit) ? $this->getSuccessJsonResponse ( [
        StringLiterals::STATUS => 'success',
        StringLiterals::MESSAGE => trans ( 'video::ads.success.updated' )
    ] ) : $this->getErrorJsonResponse ( [
        [
            StringLiterals::STATUS => 'error',
            StringLiterals::MESSAGE => trans ( 'video::ads.error.updated' )
        ]
    ] );
  }
  /**
