...
class BaseUploadRepository extends BaseRepository {
 ...
 public function defineRule() {
  ...
  /*echo "<pre>==requestParamKey===>";print_r($this->requestParamKey);echo "<======</pre>";
  echo "<pre>=====>";print_r($this->request->image->getClientMimeType());echo "<======</pre>";
  echo "<pre>=====>";print_r($this->modelIdentifier);echo "<======</pre>";
  echo "<pre>=====>";print_r($this->config);echo "<======</pre>";*/
  if($this->modelIdentifier == 'ads') {
    return $this->setRule ( $this->requestParamKey, "required|mimetypes:{$this->config->supported_format}");
  }
  else {
 ...
 protected function setTemporaryStoragePath() {
  $this->path = $this->makeOSFriendlyPath ( public_path ( (isset ( $this->config->is_file ) && ($this->config->is_file)) ? $this->config->temporary_storage_path : $this->config->temporary_image_storage_path ) );
