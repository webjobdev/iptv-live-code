                ...
                <tr data-ng-if="showRecords" data-ng-repeat="record in records track by $index" data-ng-show="showRecords" class="list-repeat" data-intialize-sidebar="">
                    ...
                    <td>
                        <div class="product_img flexbox align-items-center table-image-text"> 
                            <a ng-if="record.audio.length" href="{{url('admin/artists/audios')}}/@{{record.id}}" class="table-image-text flexbox align-items-center">
                                <div  data-ng-if="record.playlist_thumbnail.length > 0" class="image ng-scope" data-ng-src = "@{{ record.playlist_thumbnail | trusted}}" data-default-src = "{{url('contus/base/images/no-preview.png')}}">
                                </div> 
                                <div data-ng-if="record.playlist_thumbnail.length == 0" class="image ng-scope" style="background-image: url({{url('contus/base/images/no-preview.png')}})">
                                </div>                           
                                <div class="product_description tooltip-parent">
                                    <p>@{{record.playlist_name}}
                                        <span class="tooltip_title">@{{record.playlist_name}}</span>
                                    </p>
                                </div>
                            </a>
                            <a ng-if="!record.audio.length" href="javascript:void(0);" class="table-image-text flexbox align-items-center">
                                <div  data-ng-if="record.playlist_thumbnail.length > 0" class="image ng-scope" data-ng-src = "@{{ record.playlist_thumbnail | trusted}}" data-default-src = "{{url('contus/base/images/no-preview.png')}}">
                                </div> 
                                <div data-ng-if="record.playlist_thumbnail.length == 0" class="image ng-scope" style="background-image: url({{url('contus/base/images/no-preview.png')}})">
                                </div>                           
                    ...
                    <td class="table-action">
                        <div class="flexbox align-items-center justify-center">
                            <div class="column edit_table_icon tooltip-parent ">
                                <a data-boot-tooltip="true" class="table_action" href="{{url('admin/playlist/audios')}}/@{{record.id}}">
                                ...
                                <span class="tooltip_title">{{trans('audio::artists.view_artist_audios')}}</span>
                            </div>
                            <div class="column edit_table_icon tooltip-parent">
                                ...
                                <span class="tooltip_title">{{trans('base::general.view_or_edit')}}</span>
                            </div>
                            <div class="tooltip-parent">
