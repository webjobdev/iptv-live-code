<div class="tab-pane active" id="latest_video">
    ...
    <div class="table_responsive">
        <table class="table" id="fixTable" data-ng-class="{'no-records': noRecords}">
            <thead>
                <tr>
                    @include('audio::admin.common.bulkActionLayout' ,['control' => 'albumgridCtrl' ])
            ...
            <tbody>
                <tr class="search_text">
                    ...
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.album_name" data-boot-tooltip="true" title="{{trans('audio::album.enter_album_name')}}">
                    </td>
                    ...
                    <td></td>
                    <td>
                        <select data-jquery="select2_custom_ddl" minimumresults="-1"    class=" select2_custom_ddl" data-ng-change="search()" data-ng-model="searchRecords.is_active" data-ng-init="searchRecords.is_active = 'all'" data-boot-tooltip="true" title="{{trans('base::general.select_status')}}">
                            <option value="all">{{trans('base::general.all')}}</option>
                            <option value='1'>{{trans('audio::album.active')}}</option>
                            <option value='0'>{{trans('audio::album.inactive')}}</option>
                        </select>
                    </td>
                    <td></td>
                    <td>
                ...
                <tr data-ng-if="showRecords" data-ng-repeat="record in records track by $index" data-ng-show="showRecords" class="list-repeat">
                    ...
                    <td>
                        <div class="product_img flexbox align-items-center">
                            <a title="{{trans('audio::album.tooltips.view_album_audios')}}" class="table-image-text flexbox align-items-center" href="{{url('admin/albums/audios')}}/@{{record.id}}">   
                                <div class="image" data-ng-if="record.album_thumbnail.length == 0" style="background-image: url({{url('contus/base/images/no-preview.png')}})" >
                                </div>
                                <div class="image" data-ng-if="record.album_thumbnail.length > 0" style="background-image: url(@{{record.album_thumbnail}})" >
                                </div>
                                <div class="product_description">
                                    <p class="img_description">@{{record.album_name}}</p>
                                    <span class="tooltip_title ng-binding">@{{record.album_name}}</span>
                                </div>
                            </a>
                        </div>
                    </td>
                    <td><p class="grid_description">@{{record.artist[0].artist_name}}</p></td>
                    <td>@{{record.audios.length}}</td>
                    <td>@{{record.album_release_date}}</td>
                    <td>
                        <div class="tooltip-parent">
                            <span data-ng-if="record.is_active == 1 && record.audios.length > 0" ng-mouseover="getTooltip($event)" class="status-active delete_disabled" data-boot-tooltip="true">
                            {{trans('audio::artists.message.active')}}
                            </span>
                            <span class="tooltip_title">{{trans('audio::album.active_disabled')}}</span>
                        </div>
                        <div class="tooltip-parent">
                            <span class="status-active" ng-if="record.is_active == 1 && record.audios.length == 0" style="cursor: pointer;" data-ng-click="albumgridCtrl.updateStatus(record)" data-boot-tooltip="true">{{trans('base::audio.record_status.active')}}</span>
                            <span class="tooltip_title">{{trans('audio::album.tooltips.deactivate_tooltip')}}</span>
                        </div>
                        <div class="tooltip-parent">
                            <span class="status-inactive" ng-if="record.is_active != 1" style="cursor: pointer;" data-ng-click="albumgridCtrl.updateStatus(record)" data-boot-tooltip="true">{{trans('base::audio.record_status.inactive')}}</span>
                            <span class="tooltip_title">{{trans('audio::album.tooltips.activate_tooltip')}}</span>
                        </div>
                    </td>
                    <td class="table-action">
                        <div class="flexbox align-items-center justify-center">
                            <div class="column edit_table_icon tooltip-parent" style="margin-right: 17px;">
                                <a data-boot-tooltip="true" class="table_action" href="{{url('admin/albums/audios')}}/@{{record.id}}">
                            ...
                            <div class="column edit_table_icon tooltip-parent">
                                <a data-boot-tooltip="true" class="table_action" href="{{url('admin/audios/albums/edit')}}/@{{record.id}}">
                            ...
                            <div class="tooltip-parent">
                                <span data-ng-if="(record.audios.length == 0)"  ng-mouseover="getTooltip($event)" class="delete_table_icon" data-toggle="modal" data-target="#single-record-delete-popup" data-ng-click="deleteSingleRecord(record.id)"data-boot-tooltip="true">
                            ...
                            <div class="tooltip-parent">
                                <span data-ng-if="(record.audios.length > 0)" ng-mouseover="getTooltip($event)" class="delete_table_icon delete_disabled" title="" data-boot-tooltip="true">
                                <svg viewBox="0 0 11 12" x="0px" y="0px" width="11px" height="12px">
                                    <g>
                                        <path d="M 10.4998 3.513 L 9.6099 3.513 L 8.9153 11.6068 C 8.8962 11.8292 8.7144 11.9998 8.4966 11.9998 L 2.4885 11.9998 C 2.2708 11.9998 2.0889 11.8293 2.0701 11.6069 L 1.3752 3.513 L 0.4995 3.513 L 0.4995 2.6513 L 3.4268 2.6513 L 3.4268 1.7179 C 3.4268 1.322 3.741 0.9999 4.1272 0.9999 L 6.8721 0.9999 C 7.2583 0.9999 7.5725 1.322 7.5725 1.7179 L 7.5725 2.6513 L 10.4998 2.6513 L 10.4998 3.513 ZM 6.7322 1.8615 L 4.2668 1.8615 L 4.2668 2.6513 L 6.7322 2.6513 L 6.7322 1.8615 ZM 2.2188 3.513 L 2.873 11.1383 L 8.1121 11.1383 L 8.7661 3.513 L 2.2188 3.513 ZM 6.5222 9.8588 L 6.7043 4.7609 L 7.5442 4.7924 L 7.3621 9.8902 L 6.5222 9.8588 ZM 5.0796 4.7767 L 5.9199 4.7767 L 5.9199 9.8746 L 5.0796 9.8746 L 5.0796 4.7767 ZM 3.4551 4.7923 L 4.2949 4.7608 L 4.4771 9.8586 L 3.6372 9.8902 L 3.4551 4.7923 Z" fill="#454545"></path>
                                    </g>
                                ...
                                <span class="tooltip_title">{{trans('audio::album.delete_disabled')}}</span>
                            </div>
