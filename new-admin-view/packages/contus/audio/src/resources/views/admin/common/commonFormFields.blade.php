@switch($field)
    @case('order')
        <div class="form-group" data-ng-class="{'has-error': errors.order.has}">
            <label>{{trans('audio::general.order')}}</label>
            <div class="form-input">
                <input type="text" name="order" class="form-control" data-ng-model="{{$ngmodel}}" placeholder="{{trans('audio::general.order')}}" value="{{old('order')}}" />
            </div>
