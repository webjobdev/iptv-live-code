...
@section('content')
    <div data-ng-controller="AlbumGridController as albumgridCtrl" >
        <div class="page-heading flexbox align-items-center flex-wrap">
            <h4>{{trans('audio::album.manage_album')}}</h4>
            <div class="right-side flexbox align-items-center">
                <a title="{{trans('audio::album.add_album')}}" href="{{url('admin/audios/album/add')}}" class="button button-blue">
                <svg viewBox="0 0 18 18" version="1.1" x="0px" y="0px" width="18px" height="18px">
                    ...
                    <span>{{trans('audio::album.add_album')}}</span>
                </a>                
        ...
        <div class="contentpanel clearfix album_grid" >
            @include('base::partials.errors')
            <div
                data-grid-view
                data-rows-per-page="10"
                data-route-name="albums"
                data-template-route = "admin/audios/albums"
                data-request-grid="albums"
                data-count = "false"
            ></div>
        </div>
    </div>
...
<script src="{{$getAudioAssetsUrl('js/common/commonGridService.js')}}"></script> 
<script src="{{$getBaseAssetsUrl('js/gridView.js')}}"></script>
<script src="{{$getAudioAssetsUrl('js/albums/grid.js')}}"></script>
