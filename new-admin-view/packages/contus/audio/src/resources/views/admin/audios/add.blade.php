...
@section('content')
    <div ng-app="AudioAddFormApp" data-ng-controller="AudioAddFormController as audioAddCtrl">
        ...
        <!-- Album Form -->
        <div class="contentpanel form-page product order_list">
            <div class="panel album-panel">
                <div class="add_form form_container">
                    <form id="albumForm" name="albumForm" method="POST" data-base-validator data-ng-submit="audioAddCtrl.audioSave($event,'{{URL::previous()}}')"
                        enctype="multipart/form-data">
                        {!! csrf_field() !!}
                        <input type="hidden" name="isImgUpdated" id="isImgUpdated" class="form-control" data-ng-model="audioAddCtrl.albumPostData.isImgUpdated">                        
                        <div class="fixed-btm-action bottom-button buttom-button-fixed" data-ng-show="isAudioUploaded">
                            <div class="text-right btn-invoice">						
                                <button class="btn btn-primary pull-right submitbutton publish-now">{{__('base::general.submit')}}</button>
                                <a class="btn btn-danger save pull-right mr10" href="{{url('admin/audios/audios')}}">{{__('base::general.cancel')}}</a>
                            </div>
                        </div>
...
<script src="{{$getAudioAssetsUrl('js/audios/add.js')}}"></script>
