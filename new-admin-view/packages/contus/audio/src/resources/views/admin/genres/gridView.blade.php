<div id="latest_video">
    ...
    <div class="table_responsive">
        <table class="table" id="fixTable" data-ng-class="{'no-records': noRecords}">
            <thead>
                <tr>
                    @include('audio::admin.common.bulkActionLayout' ,['control' => 'genregridCtrl' ])
            ...
            <tbody>
                <tr class="search_text">
                    ...
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.genre_name" data-boot-tooltip="true" title="{{trans('audio::genres.enter_genre_name')}}">
                    </td>
                    <td></td>
                    <td>
                        <select class="form-control mb15" data-ng-change="search()" data-ng-model="searchRecords.is_active" data-boot-tooltip="true" title="{{trans('base::general.select_status')}}">
                            ...
                            <option value='1'>{{trans('audio::genres.active')}}</option>
                            <option value='0'>{{trans('audio::genres.inactive')}}</option>
                        </select>
                    </td>
                    <td></td>
                    <td></td>
                ...
                <tr data-ng-if="showRecords" data-ng-repeat="record in records track by $index" data-ng-show="showRecords" class="list-repeat" data-intialize-sidebar="">
                    ...
                    <td>
                        <p style="cursor: no-drop;" class="disabled">@{{record.genre_name}}</p>
                    </td>
                    <td>
                        <p class="grid_description">@{{record.order}}</p>
                    </td>
                    <td>
                        <span class="status-active" ng-if="record.is_active == 1" style="cursor: pointer;" data-ng-click="genregridCtrl.updateStatus(record)" title="{{trans('audio::genres.deactivate_genre')}}" data-boot-tooltip="true">{{trans('audio::genres.message.active')}}</span>
                        <span class="status-inactive" ng-if="record.is_active != 1" style="cursor: pointer;" data-ng-click="genregridCtrl.updateStatus(record)" title="{{trans('audio::genres.activate_genre')}}" data-boot-tooltip="true">{{trans('audio::genres.message.inactive')}}</span>
                    </td>
                    ...
                    <td class="table-action">
                        <div class="flexbox align-items-center justify-center">
                            <div class="column edit_table_icon">
                                <button class="table_action sidepanel-open" data-ng-click="genregridCtrl.editGenre(record)" title="{{trans('base::general.view_or_edit')}}" data-boot-tooltip="true">
                            ...
                            <span ng-mouseover="getTooltip($event)" class="delete_table_icon" title="{{trans('base::general.delete')}}" data-toggle="modal" data-target="#deleteModal" data-ng-click="deleteSingleRecord(record.id)" data-boot-tooltip="true">
