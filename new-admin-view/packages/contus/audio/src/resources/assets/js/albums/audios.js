...
var audioAlbums = angular.module('audioAlbums', []);
var commonAPP = audioAlbums;
audioAlbums.directive('baseValidator', validatorDirective);

audioAlbums.factory('requestFactory', requestFactory);

audioAlbums.controller('ViewAudioAlbumsController', ['$window', '$scope', '$rootScope', 'requestFactory', '$timeout', function (win, scope, rootScope, requestFactory, $timeout) {
  var self = this;
  scope.errors = {};
  scope.currentPage = 1;
  this.audioAlbums = {};
  ...
  this.fetchData = function (id) {
    requestFactory.get(requestFactory.getUrl('albums/audio-albums/' + id, {
      page: scope.currentPage
    }), function (response) {
      requestFactory.toggleLoader();
      this.audioAlbums = response.AudioAlbums.album;
      this.audioAlbums.audios = response.AudioAlbums.audios.data;
      this.audioAlbums.artist = response.AudioAlbums.audios.data.artist;
      this.audioAlbums.favourites_count = response.AudioAlbums.favAlbumsCount;
      scope.totalRecords = parseInt(response.AudioAlbums.audios.total);
      scope.rowsPerPage = parseInt(response.AudioAlbums.audios.per_page);
      scope.currentPage = parseInt(response.AudioAlbums.audios.current_page);
  ...
  scope.loadRecords = function (pageNumber, orderStatus) {
    ...
    self.fetchData(self.audioAlbums.id);
    requestFactory.toggleLoader();
  }
...
if (angular.isObject(window.gridControllers)) {
  for (var controller in window.gridControllers) {
    if (angular.isArray(window.gridControllers[controller]) || angular.isFunction(window.gridControllers[controller])) {
      ...
      audioAlbums.controller(controller, window.gridControllers[controller]);
    }
  }
}
/**
...
angular.element(document).ready(function () {
  angular.bootstrap(document, ['audioAlbums']);
});