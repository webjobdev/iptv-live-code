...
class SubscriptionRepository extends BaseRepository {
    ...
    public function addOrUpdateSubscriptions($id = null) {
        if (! empty ( $id )) {
            ...
            $this->setRules ( [ 'name' => 'sometimes|required|max:255','is_active' => 'sometimes|required|boolean','type' => 'sometimes|required|max:255','amount' => 'sometimes|required|between:0,99999999.99','duration' => 'sometimes|required|numeric|min:1' ] );
            $subscription->updator_id = $this->authUser->id;
        } else {
            $this->setRules ( [ 'name' => 'required|max:255','type' => 'required|max:255','amount' => 'required|between:0,99999999.99','duration' => 'required|numeric|min:1' ] );
    ...
    public function getGridHeadings() {
        if (config ()->get ( 'auth.providers.users.table' ) === 'customers') {
            ...
            return [ StringLiterals::GRIDHEADING => [ [ 'name' => trans ( 'customer::subscription.subscription_name' ),StringLiterals::VALUE => '','sort' => true,'class' => false  ],

            [ 'name' => trans ( 'customer::subscription.type' ),StringLiterals::VALUE => '','sort' => false,'class' => false  ],[ 'name' => trans ( 'customer::subscription.amount' ),StringLiterals::VALUE => '','sort' => false,'class' => true  ],[ 'name' => trans ( 'customer::subscription.duration' ),StringLiterals::VALUE => '','sort' => false,'class' => false  ],

            [ 'name' => trans ( 'customer::subscription.created_at' ),StringLiterals::VALUE => '','sort' => false,'class' => false  ],[ 'name' => trans ( 'customer::subscription.status' ),StringLiterals::VALUE => 'is_active','sort' => false,'class' => false  ],[ 'name' => trans ( 'customer::subscription.action' ),StringLiterals::VALUE => '','sort' => false,'class' => false  ] ] ];
        }
    }
