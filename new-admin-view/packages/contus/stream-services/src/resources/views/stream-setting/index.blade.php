...
@section('content')
    <div data-ng-controller="StreamSettingController as strmStngCtrl">
        <div class="" id="dashboard-page">
            <div class="page-heading flexbox align-items-center flex-wrap">
                <h4>{{ __('stream-services::index.stream_settings') }}</h4>

                {{-- <div class="right-side flexbox align-items-center">
                    <a data-ng-if="checkAccess('stream_service.create')" data-ng-click="strmStngCtrl.addStreamSetting($event)"
                        ...
                        <span>{{ trans('stream-services::index.create_stream_settings') }}</span>
                    </a>
            ...
            <form class="form-inline serch-btn" id="searchForm" method="GET" ng-submit="strmStngCtrl.searchUser($event)">
                {!! csrf_field() !!}
                <input class="form-control mr-2" type="search" ng-model="searchRecords.name" placeholder="Search here..."
                    aria-label="Search" id="searchInput" ng-change="strmStngCtrl.searchUser()">&nbsp;
                <button class="btn btn-info glyphicon glyphicon-search" type="submit"
                    style="background-color: #00ACCD;"></button>
            </form>
            <br>
            <div class="contentpanel product order_list">
                @include('base::partials.errors')
                <div class="response-msg"></div>
                <div data-grid-view data-rows-per-page="10" data-route-name="stream-services/stream-settings"
                    data-template-route="admin/stream-services/stream-settings" data-count="false"></div>
            </div>
        </div>
    </div>
@endsection
...
@section('scripts')
    ...
    <script src="{{ asset('adminview/assets/js/stream-services/streamSetting.js') }}"></script>
