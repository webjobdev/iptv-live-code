...
<div id="announcment">
    ...
    <div id="dropdowns">
        <div class="select-buttons col-sm-4">
            <!-- choose Where -->
            <select name="where" id="where" ng-model="strmStngCtrl.apiAccessData.where" class="form-control"
                style="border: 2px solid rgba(128, 130, 133, 0.36); border-radius: 20px; padding: 0px 9px;">
                <option value="">Select Stream Type
                <option>
            </select>
        </div>
        <div class="select-buttons col-sm-4">
    ...
    <div class="table_responsive">
        <table class="table subscription-plan-grid" id="fixTable" data-ng-class="{'no-records': noRecords}">
            <thead>
                <tr>
                    @include('audio::admin.common.bulkActionLayout', ['access_type' => 'stream_services'])
                    <th data-ng-repeat="field in heading" ng-class="">
                        ...
                        <span data-ng-if="field.sort==true" id="" class="th-inner sortable both"
                            ...
                            data-ng-click="fieldOrder($event,field.name)"></span>
            ...
            <tbody>
                ...
                {{-- <td></td>
                    ...
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.login"
                            data-boot-tooltip="true" data-toggle="tooltip" data-original-title="Enter login">
                    </td>
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.name"
                            data-boot-tooltip="true" data-toggle="tooltip" data-original-title="Enter name">
                    </td>
                    <td class="search_product">
                        <input type="text" class="form-control" data-ng-model="searchRecords.organization_id"
                            data-boot-tooltip="true" data-toggle="tooltip" data-original-title="Enter organization">
                    </td>
                    <td></td> --}}
                {{-- <td>
                ...
                <tr data-ng-if="showRecords" data-ng-repeat-start="record in records track by $index"
                    ...
                    <td class="">@{{ record.node }}</td>
                    <td class="">@{{ record.preset }}</td>
                    <td class="">@{{ record.status_type }}
                        <div class="text-center" id="input-btns">
                            <button data-boot-tooltip="true" data-toggle="tooltip"
                                data-original-title="https://status-output-url.test">HLS</button>
                        </div>
                    </td>
                    <td class=" text-center">
                        @{{ record.input_type }} <br>
                        <div class="text-center" id="input-btns">
                            <button data-boot-tooltip="true" data-toggle="tooltip"
                                data-original-title="https://input-url.test">HLS</button>
                            <button>HLS Push</button>
                        </div>
                    </td>
                    <td class="">@{{ record.started }}</td>
                    <td class="">@{{ record.last_reset }}</td>
                    <td class="">@{{ record.restarts }}</td>
                    <td class="">@{{ record.cpu }}</td>
                    <td class="">@{{ record.rss }}</td>
                    <td class="">@{{ record.created_by }}</td>
                    <td class="">@{{ record.status }}</td>

                    {{-- <td class="">
                        <!-- Show if active -->
                        <div class="tooltip-parent" data-ng-if="checkAccess('drm_service')">
                    ...
                    <td class="table-action">
                        <div class="flexbox align-items-center justify-center">
                            ...
                            {{-- <div data-ng-if="checkAccess('stream_services')" class="form-group row"
                                style="margin-bottom: 0px; margin-right: 5px;">
                                <label class="switch">
                                    <input type="checkbox" name="status" ng-checked="record.status == 1"
                                        ng-click="strmStngCtrl.toggleStatus(record)">
                                    <span class="slider round"></span>
                                </label>
                            </div> --}}

                            <!-- edit button -->
                            <div data-ng-if="checkAccess('stream_service.edit')" class="column edit_table_icon tooltip-parent">
                                {{-- <button data-ng-click="strmStngCtrl.editStreamSetting(record.id)"> --}}
                                <button>
