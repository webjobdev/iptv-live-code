  ...
  this.banner.is_mobile_image_updated = 0;
    scope.singleConfig= {
      ...
      load: function(query, callback) {
          ...
          $.ajax({
              url: $('meta[name="base-api-url"]').attr('content') + '/kidsbanner/searchvideos?search=' + query,
...
scope.clearbannerImage = function(){
  ...
  this.editStaticContent = function (records) {
    ...
    self.banner.video_title = records.video_title;
  ...
  this.dataSubmit = function(e, id) {
    this.save = function ($event,id) {
      ...
      if (baseValidator.validateAngularForm($event.target,scope) && !scope.errors.hasOwnProperty('banner_url') && !scope.errors.hasOwnProperty('mobile_url')) {
            if (id) {
              requestFactory.post(requestFactory.getUrl('kidsbanner/edit/' + id), this.banner, function (response) {
              ...
              requestFactory.post(requestFactory.getUrl('kidsbanner/add'), this.banner, function (response) {
                  requestFactory.setToaster('success', response.message);
                  requestFactory.getToaster();
                  $('#loaderimg').hide();                  
                  scope.getRecords(true);
    ...
    this.fillError = function(response){
     ...
     if(response.status == 422 && response.data.hasOwnProperty('message')){
        angular.forEach(response.data.message, function(message,key) {
          ...
          // if(typeof message == 'object' && message.length > 0 && key == 'banner_image'){
  ...
  this.fetchInfo = function() {
      requestFactory.get(requestFactory.getUrl('kidsbanner/info'),this.defineProperties,function(response){
  ...
  this.webUrl = function() {
    $timeout(function(){
        ...
        window.BannerImageUploadHandler.initate({
            file: 'banner-image',
            previewer: 'banner-image-preview',
  ...
  this.mobileUrl = function() {
    ...
    this.deleteBannerImage = function () {
        requestFactory.toggleLoader();
        requestFactory.post(requestFactory.getUrl('kidsbanner/delete-banner-image/' + this.banner.id), this.banner, function (response) {
            requestFactory.toggleLoader();
            requestFactory.setToaster('success', response.message);
            requestFactory.getToaster();
            scope.getRecords(true);
