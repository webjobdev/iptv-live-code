  ...
  this.banner.is_mobile_image_updated = 0;
    scope.singleConfig= {
      ...
      load: function(query, callback) {
          ...
          $.ajax({
              url: $('meta[name="base-api-url"]').attr('content') + '/banner/searchvideos?search=' + query,
...
scope.clearbannerImage = function(){
  ...
  this.dataSubmit = function(e, id) {
    this.save = function ($event,id) {
      ...
      if(this.banner.video != undefined || this.banner.video != null) {
        delete scope.errors['video'];
      }
  ...
  this.fetchInfo = function() {
      requestFactory.get(requestFactory.getUrl('banner/info'),this.defineProperties,function(response){
  ...
  this.mobileUrl = function() {
    ...
    this.deleteBannerImage = function () {
        requestFactory.toggleLoader();
        requestFactory.post(requestFactory.getUrl('banner/delete-banner-image/' + this.banner.id), this.banner, function (response) {
            requestFactory.toggleLoader();
            requestFactory.setToaster('success', response.message);
            requestFactory.getToaster();
            scope.getRecords(true);
