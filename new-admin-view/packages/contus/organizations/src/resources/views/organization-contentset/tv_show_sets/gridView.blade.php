        ...
        <table class="table subscription-plan-grid table-custom" id="fixTable"
            ...
            <thead>
                <tr>
                    ...
                    <th>S.No</th>
                    <th>Set Name</th>
                    <th>Number Of Tv Show</th>
                    <th>Number Of Tv Season</th>
                    <th>Number Of Tv Episode</th>
                    <th>Listed</th>
            ...
            <tbody>
                ...
                <tr data-ng-if="showRecords" data-ng-repeat="record in TvShowSetrecords track by $index"
                    ...
                    <td class="td-custom-width">@{{record.name}}</td>

                    <td class="td-custom-width">@{{ getTvShowCount(record) || '-'}}</td>

                    <td class="td-custom-width">@{{ getSeasonCount(record) || '-'}}</td>

                    <td class="td-custom-width">@{{ getEpisodeCount(record) || '-'}}</td>

                    <td class="td-custom-width" uib-tooltip-html="getAssignedChannels(record)" tooltip-placement="top"
                        tooltip-trigger="'mouseenter'">
                        @{{ record.assigned_tv_show.length }}
                    </td>

                    <td>
                        <div class="multi-value">
                            <div ng-if="record.monitization_type_buy == 1">Buy</div>
                            <hr>
                            <div ng-if="record.monitization_type_rent == 1">Rent</div>
                        </div>
                    </td>
                    <td>
                        <div class="multi-value">
                            <div ng-if="record.payment_method_buy == 0">Per Bundle</div>
                            <div ng-if="record.payment_method_buy == 1">Per Item</div>
                            <hr>
                            <div ng-if="record.payment_method_rent == 0">Per Bundle</div>
                            <div ng-if="record.payment_method_rent == 1">Per Item</div>
                        </div>
                    </td>
                    <td>
                        <div class="multi-value">
                            <div ng-if="record.buy_price">@{{ record.buy_price }} @{{ record.currency }}</div>
                            <hr>
                            <div ng-if="record.rent_price">@{{ record.rent_price }} @{{ record.currency }}</div>
                        </div>
                    </td>

                    <td>@{{record.getuser.email}}</td>

                    <td>@{{record.updated_at | date:'dd-MM-yyyy'}}</td>
                    ...
                    <td class="table-action">
                        <div class="flexbox align-items-center justify-center">
                            ...
                            <div data-ng-if="checkAccess('subscribers')" class="column edit_table_icon tooltip-parent">
                                <button data-ng-click="tvsetCtrl.view(record, record.id, record.organization_id)">
                            ...
                            <div data-ng-if="checkAccess('subscribers')" class="column edit_table_icon tooltip-parent">
                                <button data-ng-click="tvsetCtrl.edit(record, record.id, record.organization_id)">
