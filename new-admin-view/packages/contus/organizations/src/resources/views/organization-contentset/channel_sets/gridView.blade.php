<div id="announcment">
    ...
    <div class="table_responsive">
        <table class="table subscription-plan-grid" id="fixTable" data-ng-class="{'no-records': noRecords}">
            ...
            <!-- <pre>@{{ records | filter:{ organization_id: Id } | json }}</pre> -->
            ...
            <tbody>
                ...
                <tr data-ng-if="showRecords" data-ng-repeat="record in channelrecords track by $index"
                    ...
                    <td class="td-custom-width">@{{channelCount(record) || '-'}}</td>

                    <td class="td-custom-width" uib-tooltip-html="getAssignedChannels(record)" tooltip-placement="top"
                        tooltip-trigger="'mouseenter'">
                        @{{ record.assigned_channels.length }}
                    </td>


                    <td class="td-custom-width">@{{record.monitization_type == 0 ? 'Rent' : 'Buy'}}</td>
                    <td>@{{record.payment_method == 0 ? 'Per Bundel' : 'Per Item'}}</td>
                    <td>@{{record.price}}</td>
                    ...
                    <td class="table-action">
                        <div class="flexbox align-items-center justify-center">
                            ...
                            <div data-ng-if="checkAccess('subscribers')" class="column edit_table_icon tooltip-parent">
                                <button data-ng-click="setCtrl.view(record, record.id, record.organization_id)">
                            ...
                            <div data-ng-if="checkAccess('subscribers')" class="column edit_table_icon tooltip-parent">
                                <button data-ng-click="setCtrl.edit(record, record.id, record.organization_id)">
