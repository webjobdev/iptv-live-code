...
<div class="form-group row" style="margin-bottom: 15px;">
    <label for="prefix" class="col-sm-10 control-label" style="font-size: 14px; color: #000; margin-top: 10px;">
        Tv Show<span class="required">*</span>:
    </label>
    <label>Show In Liv Mode</label>
    <label class="switch" style="margin: 10px 0px 10px 16px;">
        <input type="checkbox" ng-model="fturow.show_in_live" name="show_in_live"
            ng-checked="record.content_sets.show_in_live == 1"
            style="border: 2px solid rgba(128, 130, 133, 0.36); border-radius: 20px; padding: 0px 9px; height: auto;">
        <span class="slider round"></span>
    </label>

    <div class="channel-wrapper">
        <div class="channel-card" ng-repeat="bundle in record.content_sets.tvShows" data-id="@{{bundle.id}}"
            ng-model="fturow.tvShowContentSet">
            ...
            <div class="card-footer">
                ...
                <span class="remove-btn" data-ng-click="fturCtrl.removeTvShowBundle(bundle)">−</span>
            </div>
        </div>



        <div class="add-card" data-toggle="modal" data-target="#assigned-show"
            ng-click="fturCtrl.openTvShowModal(record)">
            + Add Channels
        </div>
    </div>
</div>
