        ...
        <table class="table subscription-plan-grid table-custom" id="fixTable"
            ...
            <tbody>
                <tr class="search_text">
                    <td></td>
                    <td></td>
                    <td class="search_subscription">
                        <input type="text" class="form-control" data-ng-model="searchRecords.subscription_name"
                            data-boot-tooltip="true" title="Type subscription Name"
                            placeholder="Type subscription Name">
                    </td>
                    <td></td>
                    <td>
                        <select class="select2_custom_ddl" minimumResults="-1" data-jquery="select2_custom_ddl"
                            data-boot-tooltip="true" data-ng-model="searchRecords.platforms" data-ng-change="search()"
                    ...
                    <td>
                        <select class="select2_custom_ddl" minimumResults="-1" data-jquery="select2_custom_ddl"
                            data-boot-tooltip="true" data-ng-model="searchRecords.autopay" data-ng-change="search()"
                            data-toggle="tooltip">
                            {{-- <option value="" disabled selected> Select Autopay</option> --}}
                    ...
                    <td></td>
                    <td class="search_subscription">
                        <input type="text" class="form-control" data-ng-model="searchRecords.user"
                            data-boot-tooltip="true" title="Type User's Email" placeholder="Type User's Email">
                    </td>
                    <td></td>
                </tr>
                ...
                <tr data-ng-if="showRecords" data-ng-repeat="record in MonPlanrecords track by $index"
                    ...
                    <td class="td-custom-width">
                        @{{ record.subscription_length ? record.subscription_length : record.subs_length_time_type }}
                    </td>

                    <td class="td-custom-width">
                        <div class="badge-container">
                            <span class="badge bg-dark" ng-if="record.platforms"
                                ng-repeat="(key, value) in record.platforms" ng-if="value">
                                @{{ key }}
                            </span>

                            <span class="badge bg-dark" ng-if="record.use_org_settings == 1">
                                Use Org Settings
                            </span>
                    ...
                    <td class="td-custom-width">
                        @{{ record.currency || 'Free' }}
                    </td>

                    <td class="td-custom-width">
                        @{{ record.autopay == '0' ? 'Not Allowed' : 'Allowed' }}
                    </td>

                    <td class="td-custom-width">
                        @{{ record.subscription_devices || 'Free' }}
                    </td>
                    <td class="td-custom-width">
                        @{{ record.total_price || 'Free' }}
                    </td>
                    <td class="td-custom-width">
                        @{{ record.user.email }}
                    </td>
                    ...
                    <td class="table-action">
                        <div class="flexbox align-items-center justify-center">
                            ...
                            <div class="form-group row" style="margin-bottom: 0px; margin-right: 5px;">
                                <label class="switch">
                                    <input type="checkbox" ng-checked="record.is_active == 1"
                                        ng-click="subscrCtrl.togglePublishNow(record, record.id)">
                                    <span class="slider round"></span>
                                </label>
                            </div>

                            <!-- edit button (class="table_action sidepanel-open")-->
                            <div data-ng-if="checkAccess('monetization_plans.edit')"
                                class="column edit_table_icon tooltip-parent">
                                <button data-ng-click="subscrCtrl.editData(record)">
                            ...
                            <!-- delete button -->
                            <div data-ng-if="checkAccess('monetization_plans.delete')" class="tooltip-parent">
