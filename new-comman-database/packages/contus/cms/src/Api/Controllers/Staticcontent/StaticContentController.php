...
class StaticContentController extends ApiController {
    ...
    public function getSiteFooter() {
        $data = $this->repository->getFooterContents ();
        return ($data) ? $this->getSuccessJsonResponse ( [ 'response' => $data ] ) : $this->getErrorJsonResponse ( [ ], 422);
    }
    /**
    ...
    public function getMenuCategories() {
        $data = $this->repository->getMainmenuCategories ();
        return ($data) ? $this->getSuccessJsonResponse ( [ 'response' => $data ] ) : $this->getErrorJsonResponse ( [ ], 422);
    }

    public function getAudioSiteFooter() {
        $data = $this->repository->getAudioFooterContents ();
        return ($data) ? $this->getSuccessJsonResponse ( [ 'response' => $data ] ) : $this->getErrorJsonResponse ( [ ], 422);
    }
