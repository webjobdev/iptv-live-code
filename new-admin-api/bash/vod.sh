...
target="${3}"
ffmpegPath="/opt/kaltura/bin/ffmpeg"
ffProbePath="/opt/kaltura/bin/ffprobe"
if [[ ! "${target}" ]]; then
...
for rendition in "${renditions[@]}"; do
  ...
  then
   ...
   # cmd+=" ${static_params} -vf scale=w=${width}:h=${height}:force_original_aspect_ratio=decrease"
    cmd+=" ${static_params} -vf scale=trunc(iw/2)*2:trunc(ih/2)*2 -preset slow"
    cmd+=" -b:v ${bitrate} -maxrate ${maxrate%.*}k -bufsize ${bufsize%.*}k -b:a ${audiorate}"
    cmd+=" -hls_segment_filename ${target}/${name}_%03d.ts ${target}/${name}.m3u8"
    
    # add rendition entry in the master playlist
    master_playlist+="#EXT-X-STREAM-INF:BANDWIDTH=${bandwidth},RESOLUTION=${resolution},CLOSED-CAPTIONS=NONE\n${name}.m3u8\n"
  fi
done
