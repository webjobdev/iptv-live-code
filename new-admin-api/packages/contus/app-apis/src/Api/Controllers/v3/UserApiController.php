...
class UserApiController extends AppApiController
{
    ...
    public function addToMyList(Request $request)
    {
        ...
        if ($request->type == 'movie') {
            ...
            $myList->video_id = $request->rec_id;
        }
        $myList->save();
    ...
    public function showSuratCountry(Request $request)
    {
        $lat = $request->input('lat', '34.052002');
        $lon = $request->input('lon', '-118.243683');

        $response = Http::withHeaders(['User-Agent' => 'LaravelApp'])
