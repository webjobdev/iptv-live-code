...
namespace Contus\AppApi\Api\Controllers\App;
...
class AppApiController extends ApiController
{
    ...
    public function SeasonDetail(Request $request)
    {
        ...
        $trailer = TvShow::where('id', '!=', $request->id)
            ->whereHas('getAllOrganization', function ($query) use ($user) {
                $query->where('organization_id', $user->organization_id);
            })
            ->value('trailer_url');
    ...
    //     // If NO category is passed → return all channels
    //     if (!$request->category) {
    //         $allChannels = Channel::where('organization', $user->organization_id)->get();
    ...
    //     $category = TvCategory::where('organization', $user->organization_id)
    //         ->where('tv_categorie_name', $request->category)
    //         ->whereHas('children.getChannel.channelDetail', function ($q) use ($user) {
    //             $q->where('organization', $user->organization_id);
    //         })
    //         ->with([
    //             'children.getChannel.channelDetail' => function ($q) use ($user) {
    //                 $q->where('organization', $user->organization_id);
    //             }
    //         ])
    //         ->first();
    ...
    //         return response()->json([
    //             'success' => false,
    //             'message' => "Category not found your organization",
    //             // 'data' => []
    //         ], 404);
    //     }

    //     // Extract and attach "type"
    //     $channels = [];

    //     foreach ($category->children as $cat) {
    //         if (
    //             $cat->getChannel &&
    //             $cat->getChannel->channelDetail &&
    //             $cat->getChannel->channelDetail->organization == $user->organization_id
    //         ) {
    //             $channel = $cat->getChannel->channelDetail;

    //             // Convert to array
    //             $channelArr = $channel->toArray();

    //             // ADD TYPE HERE
    //             $channelArr["category"] = $request->category;

    //             // Push final formatted channel
    //             $channels[] = $channelArr;
    //         }
    //     }
    ...
    public function fetchLiveChannel(Request $request)
    {
        ...
        foreach ($category->children as $subCategory) {
            foreach ($subCategory->subCategories as $channelRow) {
                ...
                // Check organization via relationship if needed, 
                // but relying on the eager load filter above is safer/cleaner.
                // If you want double check manually:
                // if (!$channel->getAllOrganization->contains('id', $user->organization_id)) {
                //      continue;
                // }
    ...
    public function liveChannelCategories(Request $request)
    {
        ...
        // $categories = TvCategory::where('organization', $user->organization_id)
        //     ->with(['categorie_id.getChannel.channelDetail', 'getOrganization'])
        // ->whereNull('categorie_id')
        // ->get();
        ...
        //     $category->categorie_id->each(function ($subCat) {
        //         $subCat->get_sub_category->each(function ($child) {
        //             if ($child->getChannel) {
        //                 $child->getChannel->makeHidden('content_sets');
        //             }
        //         });
        //     });
        // });
