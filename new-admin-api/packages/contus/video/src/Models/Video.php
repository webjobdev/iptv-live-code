...
class Video extends Model implements AttachableModel
{
     ...
     * @var array
     */
    protected $appends = ['is_demo', 'channel_sets'];
    /**
     ...
     * @var array
     */
    protected $url = ['thumbnail_image', 'poster_image', 'mobile_poster_image', 'selected_thumb'];
    ...
    {
        parent::__construct();
        $this->setHiddenCustomer(['notification_status', 'aws_prefix', 'is_hls', 'video_url', 'pipeline_id', 'preview_image', 'subscription', 'job_id', 'country_id', 'is_subscription', 'is_featured', 'trailer', 'disclaimer', 'subtitle_path', 'subtitle', 'thumbnail_path', 'creator_id', 'updator_id', 'updated_at', 'archived_on', 'fine_uploader_uuid', 'fine_uploader_name', 'youtubePrivacy', 'liveStatus', 'pivot', 'youtube_id', 'nextPageToken', 'totalResults']);
    }
    ...
    {
        return $this->belongsToMany(Organization::class, 'live_event_organization', 'live_event_id', 'organization_id');
    }
    ...
    {
        return $this->hasMany(Comment::class, 'video_id')->whereNull('parent_id')->orderBy('_id', 'desc');
    }
    ...
    /**
     * belongsToMany relationship between collection video and genre
     */

    public function genrecollections()
    {
        return $this->belongsToMany(Group::class, 'collections_videos', StringLiterals::VIDEOID, 'group_id');
    }
    ...
    /**
     * HasMany relationship between videos and user
     */
    ...
    {
        return $this->belongsToMany(Season::class, 'video_seasons', StringLiterals::VIDEOID, 'season_id');
    }


    // fetch liveevent content set from organizations
    public function getChannelSetsAttribute()
    {
        ...
        return collect($contentSets)->map(function ($org) {
            ...
            $ids = $org['liveevent_contentset'] ?? [];

            // Fetch actual records from DB (only id, name or any other field)
            $records = LiveEventContent::whereIn('id', $ids)
                ->get()
                ->each(function ($model) {
