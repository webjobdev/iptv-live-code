...
class YoutubeRepository extends BaseRepository {
    ...
    public function __construct() {
        ...
        if(file_exists(storage_path ( "client_secret.json" )) && !empty($this->clientSecretPath)) {
    ...
    public function youtubeListUploadsChannel($channel, $nextpage = '') {
        ...
        foreach ( $streamsResponse->items as $live ) {
            ...
            if (empty ( $newLive ['id'] )) {
                ...
                $newLive->is_active = 1;
                $newLive->creator_id = \Auth::user()->id;
                $newLive->updator_id = \Auth::user()->id;
            }
    ...
    public function import() {
        ...
        $getDYVideos = $getDYVideos->where ( 'youtube_id', '!=', '' )->where ( [ 'is_live' => 0,'job_status' => 'Added' ] )->get ();
