...
class LiveeventsRepository extends BaseRepository implements IVideoRepository
{
    ...
    public function addVideo()
    {
        ...
        $liveVideo = new Video();
        $liveVideo->creator_id = \Auth::user()->id;
        ...
        $liveVideo->updator_id = \Auth::user()->id;
        ...
        if ($checkLive) {
            ...
            return [
                'heading' => [
                    ...
                    ['name' => trans('video::videos.streaming_url_policy'), 'value' => '', 'sort' => true],
                    ['name' => trans('video::videos.listed'), 'value' => '', 'sort' => true],
                    ['name' => trans('video::videos.catch_up'), 'value' => '', 'sort' => true],
                    ['name' => trans('video::videos.live_rewind'), 'value' => '', 'sort' => true],
                    ...
                    ['name' => 'Added Date', 'Added Date' => '', 'sort' => false],
                    ['name' => 'Publish Date', 'Publish Date' => '', 'sort' => false],
                    ['name' => 'Live Event Start Date', 'Live Event Start Date' => '', 'sort' => false],
                    ['name' => 'Unpublish Date', 'Unpublish Date' => '', 'sort' => false],
                    ['name' => trans('video::videos.purchased'), 'value' => '', 'sort' => true],
                    ['name' => trans('video::videos.state'), 'value' => '', 'sort' => true],
    ...
    public function prepareGrid()
    {
         ...
         */
        $this->setGridModel($this->video)->setEagerLoadingModels([
    ...
    protected function updateGridQuery($builder)
    {
        ...
        return $builder->selectRaw('videos.*, videos.id as favourite_count, videos.id as comments_count,  videos.id as like_count, videos.id as dislike_count, videos.id as is_show_hide_comment, videos.id as is_show_hide_likes, videos.id as formatted_created_date, videos.id as formatted_published_date,videos.id as is_banner,videos.id as event_start_date,videos.id as event_end_date')
            ->groupBy('videos.id')
            ->with(['categories', 'user'])
            ->where(StringLiterals::IS_ARCHIVED, 0)
            ->where('is_live', 3);
    ...
    protected function searchFilter($builderLiveVideos)
    {
        ...
        if (is_numeric($is_active)) {
            ...
            $builderLiveVideos = $builderLiveVideos->where(StringLiterals::ISACTIVE, $is_active)->where('is_live', 3);
        }
        /**
