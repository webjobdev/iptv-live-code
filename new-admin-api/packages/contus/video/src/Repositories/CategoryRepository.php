...
class CategoryRepository extends BaseRepository implements ICategoryRepository
{
    ...
    public function addOrUpdateCategory($id = null)
    {
        ...
        if ($this->request->image && $this->request->is_image_updated == 1) {
            ...
            $localStoragePath = config("app.url") . config("contus.base.image.category_image.temporary_image_storage_path");
            ...
            $category->image_url = $localImgURL;
        }

        if ($this->request->mobileimage && $this->request->is_mobile_image_updated == 1) {
            $this->deleteMobileCategoryImage($id);
            $fileName = $category->getImageBaseName($this->request->mobileimage);
            // $folderName = config("contus.base.image.mobile_category_image.s3_location");
            $localStoragePath = config("app.url") . config("contus.base.image.mobile_category_image.temporary_image_storage_path");
            // $s3BucketImgFilename = $this->uploadTos3Bucket($fileName, $folderName, $localStoragePath);
            $localImgURL = $localStoragePath . DIRECTORY_SEPARATOR . $fileName;
            $category->mobile_url = $localImgURL;
        }
    ...
    public function deleteMobileCategoryImage($id)
    {
        /**
         * Check if category id exists.
         */
        if (!empty($id)) {
            $category = $this->_category->findorfail($id);
            $categoryImage = $category->mobile_url;
            if (!empty($categoryImage)) {
                $URL = $category->getImageBaseName($category->mobile_url);
                /** call to method to delete image in S3 bucket */
                $imageURL = config("contus.base.image.mobile_category_image.s3_location") . $URL;
                $this->uploadRepository->deleteFileFromS3Bucket($imageURL);
                /** Process to delete image from local storage path */
                $localFilePath = public_path() . DIRECTORY_SEPARATOR . config("contus.base.image.mobile_category_image.temporary_image_storage_path");
                $this->uploadRepository->deleteImageFileInLocalPath($localFilePath, $URL);
                /**
                 * Empty the mobile_url and image_path field in the database.
                 */
                $category->mobile_url = '';
                $category->save();
                return true;
    ...
    {
        $categories = Category::where('parent_id', 0)->where('is_radio', 0)->where('is_web_series', 0)->where('is_deletable', 1)->select('id', StringLiterals::TITLE)->get();
        $categoryList = '<ul>';
    ...
    {
        return $this->_category->selectRaw('id, title')->has('child_category', '>', 0)->where('parent_id', 0)->where('is_web_series', 0)->where('is_active', 1)->where('is_radio', 0)->with('child_category.child_category')->get()->toArray();
    }

    public function getAllRadioCategoryInfo($slug = '')
    {
        return $this->_category->selectRaw('id, title')->where('parent_id', 0)->where('is_web_series', 0)->where('is_active', 1)->where('is_radio', 1)->get()->toArray();
    }

    public function getAllLiveCategoryInfo($slug = '')
    {
        return $this->_category->selectRaw('id, title')->where('parent_id', 0)->where('is_web_series', 0)->where('is_active', 1)->where('is_radio', 2)->get()->toArray();
    }
    ...
    protected function updateGridQuery($builder)
    {
        ...
        $builder->selectRaw('categories.*, categories.id as formatted_created_date')->where('is_deletable', 1)->where('is_radio', 0);
        return $builder;
    }
