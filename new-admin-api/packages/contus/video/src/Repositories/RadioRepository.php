...
class RadioRepository extends BaseRepository implements IVideoRepository
{
    ...
    public function getGridHeadings()
    {
        ...
        if ($checkLive) {
            ...
            return [
                ...
                ['name' => 'Website Status', trans('video::videos.value') => 'hide_web', 'sort' => false],
    ...
    protected function updateGridQuery($builder)
    {
        ...
        return $builder->selectRaw('videos.*, videos.id as favourite_count, videos.id as comments_count,  videos.id as like_count, videos.id as dislike_count, videos.id as is_show_hide_comment, videos.id as is_show_hide_likes, videos.id as formatted_created_date, videos.id as formatted_published_date,videos.id as is_banner')->groupBy('videos.id')->with(['categories', 'user'])->where(StringLiterals::IS_ARCHIVED, 0)->where('is_live', 2);
    ...
    protected function searchFilter($builderLiveVideos)
    {
        ...
        $title =  $hide_web = $is_active = $type =  $category = null;
        ...
        if (is_numeric($hide_web)) {
           
            $builderLiveVideos = $builderLiveVideos->where('hide_web', $hide_web)->where('is_live',2);
        }

        if (is_numeric($is_active)) {
           
            $builderLiveVideos = $builderLiveVideos->where(StringLiterals::ISACTIVE, $is_active)->where('is_live',2);
        }
