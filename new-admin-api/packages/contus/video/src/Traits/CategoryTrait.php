...
trait CategoryTrait {
    ...
    public function getChildCategoryEach($value) {
        ...
        foreach ( $value ['child_category'] as $newvalue ) {
            if (config ()->get ( 'auth.providers.users.table' ) === 'customers') {
                    ...
                    $subcatvalue [$newvalue ['id']]['id'] = $newvalue ['id'] ;
                    $subcatvalue [$newvalue ['id']]['parent'] = $value ['title'] ;
                    $subcatvalue [$newvalue ['id']]['child'] = $newvalue ['title'];
    ...
    public function getRelatedVideoSlug($slug, $getCount = 10, $paginate = true) {
        ...
        if ($this->request->header ( 'x-request-type' ) == 'mobile') {
            ...
            } )->selectRaw ( 'videos.*, videos.id as is_favourite, videos.id as collection' )->groupBy ( 'videos.id' )->with ( [ 'categories' ] )->where ( 'is_live', '==', 0 )->orderBy ( 'video_order', 'asc' );
            if ($paginate) {
            ...
            if (Cache::has ( 'relatedCategoryList' . $cat->slug ) || $this->request->input ( 'page' ) > 1) {
                return Cache::rememberForever ( 'relatedCategoryList' . $cat->slug, function () use ($cat, $slug) {
                    ...
                    $video = $video->with ( [ 'categories' ] )->where ( 'videos.' . $this->getKeySlugorId (), '!=', $slug )->where ( 'is_live', '==', 0 )->orderBy ( 'video_order', 'asc' )->get ();
                    return [ 'data' => $video->toArray (),'next_page_url' => null,'total' => count ( $video ) ];
                } );
            } else {
                $video = $video->first ()->videos ()->selectRaw('videos.*, videos.id as is_favourite, videos.id as collection')->where ( 'videos.' . $this->getKeySlugorId (), '!=', $slug )->orderBy ( 'video_order', 'asc' );
                $video = $video->with ( [ 'categories' ] )->where ( 'is_live', '==', 0 )->orderBy ( 'video_order', 'asc' );
                if ($paginate) {
    ...
    public function browsepreferenceListPlaylist() {
        $customer_preferences = MypreferencesVideo::where ( 'user_id', \Auth::user()->id )->pluck( 'category_id' )->toArray ();
    ...
    public function getMainCategory() {
        return Category::where('parent_id', 0)->where('level', 0)->where('is_active', 1)->orderBy('category_order', 'asc')->paginate(5);
    }
}