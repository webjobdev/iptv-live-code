 ...
 */
    ...
    public function postAction()
    {
        if ($this->request->has(StringLiterals::SELECTED_CHECKBOX) && is_array($this->request->get(StringLiterals::SELECTED_CHECKBOX))) {
            ...
            return $isActionCompleted ? $this->getSuccessJsonResponse([], trans('video::videos.webseries.message.bulk-delete-success')) : $this->getErrorJsonResponse([], trans(StringLiterals::INVALID_REQUEST_TRANS), 403);
        }
    }
    ...
    public function postBulkUpdateStatus()
    {
        if ($this->request->has(StringLiterals::SELECTED_CHECKBOX) && is_array($this->request->get(StringLiterals::SELECTED_CHECKBOX))) {
            if ($this->request->get('status') === 1) {
                ...
                return $isActionCompleted ? $this->getSuccessJsonResponse([], trans('video::videos.webseries.message.bulk-activate')) : $this->getErrorJsonResponse([], trans(StringLiterals::INVALID_REQUEST_TRANS), 403);
                ...
                return $isActionCompleted ? $this->getSuccessJsonResponse([], trans('video::videos.webseries.message.bulk-deactivate')) : $this->getErrorJsonResponse([], trans(StringLiterals::INVALID_REQUEST_TRANS), 403);
            }
        }
    }
