...
class LiveStreamController extends ApiController
{
     ...
     * Construct method
     */
    public function __construct(LiveStreamRepository $livestreamrepository)
    {
        parent::__construct();
    ...
    {
        $v = $this->repository->liveradio();
        if ($v == 'success') {
            return $this->getSuccessJsonResponse(['message' => 'Radio added successfully']);
        } else {
            return $this->getErrorJsonResponse([], $v);
        }
    }
    ...
    {
        $v = $this->repository->liveevent();
        if ($v == 'success') {
            return $this->getSuccessJsonResponse(['message' => 'Event added successfully']);
        } else {
            return $this->getErrorJsonResponse([], $v);
        }
    }
    ...
    {
        $response = $this->repository->startLiveStreamRepository();
        return $this->getSuccessJsonResponse(['info' => $response]);
    }
    ...
    {
        $response = $this->repository->stopLiveStreamRepository();
        return $this->getSuccessJsonResponse(['info' => $response]);
    }
    ...
    public function togglePublishNow($id)
    {
        ...
        if ($this->repository->togglePublishNow($id)) {
            $isUpdated = true;
            return ($isUpdated) ?
                $this->getSuccessJsonResponse(['message' => 'Live Stream Data Update Successfully.']) :
                $this->getErrorJsonResponse([], 'Live Stream Data Not Update.');
        }
    }
}
