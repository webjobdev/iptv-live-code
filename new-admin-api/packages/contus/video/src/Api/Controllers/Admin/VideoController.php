...
class VideoController extends ApiController
{
     ...
     */
    public function __construct(VideoRepository $videosRepository, CollectionRepository $collectionsRepository, CategoryRepository $categoryRepository, UploadRepository $uploadRepository, CountriesRepository $countriesRepository, VideoCountriesRepository $videoCountriesRepository, SeasonRepository $seasonRepository)
    {
        parent::__construct();
    ...
    public function postAdd()
    {
         ...
         */
        return ($isCreated) ? $this->getSuccessJsonResponse(['video' => $videoId, 'language' => SiteLanguage::where('is_active', 1)->get()->toArray(), StringLiterals::MESSAGE => trans('video::videos.message.success')]) : $this->getErrorJsonResponse([], trans('video::videos.message.error'));
    }

    public function Add()
    {
        $isCreated = false;
        if ($this->repository->add()) {
            $isCreated = true;
    ...
    public function postEdit($id)
    {
        ...
        return ($isUpdated)
            ? $this->getSuccessJsonResponse([StringLiterals::MESSAGE => trans('video::videos.message.update-success')])
            : $this->getErrorJsonResponse([], trans('video::videos.message.update-error'));
    }
    ...
    public function postEditVideo($id)
    {
        ...
        try {
            /**
             * Call editVideo repository method to update details of a video.
             */
            if ($this->repository->editVideo($id)) {
                $isUpdated = true;
            }
        } catch (Exception $e) {
            /**
    ...
    /**
     * Function to add language data for title, description and starring
     *
     * @param int $id
    ...
    public function addLanguage($id)
    {
        ...
        try {
             ...
             */
            if ($this->repository->updateVideoLanguage($id)) {
                $isUpdated = true;
    ...
    public function addLanguageToMultipleVideos($id)
    {
        ...
        try {
             ...
             * Call updateVideo repository method to update details of a video.
             */
            if ($this->repository->updateMultipleVideoLanguage($id)) {
                $isUpdated = true;
                $this->request->session()->flash(StringLiterals::SUCCESS, trans('video::videos.message.update-success'));
            }
        } catch (Exception $e) {
     ...
     * @return \Illuminate\Http\Response
     */
    public function getGeoInfo($id)
    {
        $geoSetting = GeoSettings::where('is_active', true)->first();
        $selectedCountries = $this->repository->getGeoInfo($id);

        return $this->getSuccessJsonResponse([
            'info' => [
                'geo_setting' => $geoSetting,
            ...
            [
                'info' =>
                    [
                        ...
                        'allRdioCategories' => $this->categoryRepository->getAllRadioCategoryInfo(),
                        'allLiveCategories' => $this->categoryRepository->getAllLiveCategoryInfo(),
                        'allSeries' => $this->categoryRepository->getAllSeries(),
                        'transcodedInfo' => $this->repository->getTranscodedHours(),
                        'allSeasons' => $this->seasonRepository->getAllSeasons(),
                        ...
                        'ads_info' => Ads::SelectRaw('id, title, ads_url')->where('is_active', 1)->where('ads_url', '!=', '')->get()->toArray(),
                        'cast_info' => Cast::SelectRaw('id, name, banner_image')->where('is_active', 1)->get()->toArray()
                    ]
            ]
        );
    }
        ...
        if ($this->request->has(StringLiterals::SELECTED_CHECKBOX) && is_array($this->request->get(StringLiterals::SELECTED_CHECKBOX))) {
            ...
            if ($this->request->get('videoStatus') == 'single-video') {
                $result = $isActionCompleted
                    ? $this->getSuccessJsonResponse([], trans('video::videos.message.delete-success'))
                    : $this->getErrorJsonResponse([], trans(StringLiterals::INVALID_REQUEST_TRANS), 403);
            } elseif ($this->request->get('videoStatus') == 'bulk-video') {
                $result = $isActionCompleted
                    ? $this->getSuccessJsonResponse([], trans('video::videos.message.bulk-delete-success'))
                    : $this->getErrorJsonResponse([], trans(StringLiterals::INVALID_REQUEST_TRANS), 403);
            }
        }
     ...
     */
    public function getWebSeriesCategoryInfo()
    {
        $webseries_categories = $this->categoryRepository->getWebSeriesCategoryInfo();
        return $this->getSuccessJsonResponse([
            'webseries_categories' => $webseries_categories
        ]);
    }
    ...
    public function postThumbnail()
    {
        ...
        $tempImageInfo = $this->uploadRepository->setModelIdentifier(UploadRepository::MODEL_IDENTIFIER_THUMBNAIL)->tempPrepare()->tempUpload($moduleName, $this->request->size);
    ...
    {
        $getVideoId = $this->repository->getVideoId($id);
        return (is_null($getVideoId)) ? $this->getErrorJsonResponse([], null, 404) : $this->getSuccessJsonResponse(['response' => $getVideoId]);
    }
    ...
    {
        $moduleName = 'video-image';
        $tempImageInfo = $this->uploadRepository->setModelIdentifier(UploadRepository::MODEL_IDENTIFIER_POSTER)->tempPrepare()->tempUpload($moduleName, $this->request->size);
    ...
    public function postMobilePosters()
    {
        $moduleName = 'video-image';
        $tempImageInfo = $this->uploadRepository->setModelIdentifier(UploadRepository::MODEL_IDENTIFIER_MOBILE_POSTER)->tempPrepare()->tempUpload($moduleName, $this->request->size);
    ...
    {
        $videoObj = Video::selectRaw('id, aws_prefix')->where('is_active', '1')->where('is_archived', 0)->where('job_status', 'Complete')->where('aws_prefix', '!=', '')->get();

        foreach ($videoObj as $obj) {
            $aws_prefix = $obj->aws_prefix;
            $prefix = $obj->aws_prefix;
            ...
            foreach ($formats as $format) {
                ...
                $formatResultBody = str_replace('api.demo.mazin.vplayed.com', 'api.demo.breathe.vplayed.com', $formatResultBody);
    ...
    {
        $result = $this->repository->generateMetaData($this->request);
    }

    public function getvideoPerformanceStatistics()
    {

        return $this->repository->getvideoPerformanceStatistics($this->request);
    }

    public function getvideoGeographicStatistics()
    {
        return $this->repository->getvideoGeographicStatistics($this->request);
    }

    /** Method to update the transcode status to the videos table
     *
     * @param $id
     * @package Video
     * @return response
    ...
    {
        $data = $this->repository->postTranscodeStatus($id);
        return ($data) ? $this->getSuccessJsonResponse(['status' => 'Success', 'message' => trans('video::videos.transcode_success.updated')]) : $this->getErrorJsonResponse([], trans('video::videos.transacode_status.failed'));
    }
    /**
     * Method to get the access level of the current logged in user
     *
     * @return json
     */
    public function handleAccessLevel()
    {
        $userPermissions = check_menu_flag();
        return ($userPermissions) ? $this->getSuccessJsonResponse(['data' => ['permissions' => $userPermissions]], "Access level successfully fetched")
            : $this->getErrorJsonResponse([], "User not authorized");
    }
    public function getSearchExportData()
    {
        return $this->repository->bulkExportSearchData();
    }
    ...
    //         $isUpdated = true;
    //         return ($isUpdated) ?
    //             $this->getSuccessJsonResponse(['message' => 'Video Data Update Successfully.']) :
    //             $this->getErrorJsonResponse([], 'Video Data Not Update.');
    //     }
    // }
}
