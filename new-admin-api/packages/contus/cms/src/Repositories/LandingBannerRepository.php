...
use Contus\Cms\Models\LandingBanner;
...
class LandingBannerRepository extends BaseRepository {
     ...
     */
    public function __construct(LandingBanner $bannerContent, Video $video) {
        parent::__construct ();
    ...
    public function addOrUpdateBannerContents($id = null) { 
        if (! empty ( $id )) {
            ...
            $bannerContent = new LandingBanner(); 
        }
        ...
        if($this->request->has('video')) {
            ...
            $bannerContent->video_id= '0'; 
        }
        // $bannerContent->is_status = 0;
        $bannerContent->updated_at = $currendate; 
        $bannerContent->title= $this->request->title; 
        $bannerContent->url = $this->request->imageUrl; 
        $bannerContent->type = $this->request->type; 
        $bannerContent->banner_order = $this->request->banner_order; 
        ...
        if ($this->request->banner_image && $this->request->is_image_updated == 1) {
            ...
            $folderName = config("contus.base.image.web_image.s3_location");
            $localStoragePath = public_path().DIRECTORY_SEPARATOR.config("contus.base.image.web_image.temporary_image_storage_path");
        ...
        if ($this->request->mobile_image && $this->request->is_mobile_image_updated == 1) {
            $this->deleteMobileBannerImage($id);
            $fileName =  $bannerContent->getImageBaseName($this->request->mobile_image);
            $folderName = config("contus.base.image.mobile_image.s3_location");
            $localStoragePath = public_path().DIRECTORY_SEPARATOR.config("contus.base.image.mobile_image.temporary_image_storage_path");
            $s3BucketImgFilename = $this->uploadTos3Bucket($fileName,$folderName,$localStoragePath);
            $s3BucketImgURL = $folderName.$s3BucketImgFilename;
            $bannerContent->mobile_image =  $s3BucketImgURL;
        }
    ...
    public function deleteBannerImage($id){
         ...
         */
         if (!empty ($id)) {
            ...
            if(!empty($bannerImage)){
                ...
                $imageURL = config("contus.base.image.web_image.s3_location").$URL;
                $this->uploadRepository->deleteFileFromS3Bucket($imageURL);
                /** Process to delete image from local storage path */
                $localFilePath = public_path().DIRECTORY_SEPARATOR.config("contus.base.image.web_image.temporary_image_storage_path");
     ...
     public function deleteMobileBannerImage($id){
        /**
         * Check if category id exists.
         */
         if (!empty ($id)) {
            $banner =  $this->_banner->findorfail($id);
            $bannerImage = $banner->mobile_image;
            if(!empty($bannerImage)){
                $URL = $banner->getImageBaseName($banner->mobile_image);
                /** call to method to delete image in S3 bucket */
                $imageURL = config("contus.base.image.mobile_image.s3_location").$URL;
                $this->uploadRepository->deleteFileFromS3Bucket($imageURL);
                /** Process to delete image from local storage path */
                $localFilePath = public_path().DIRECTORY_SEPARATOR.config("contus.base.image.mobile_image.temporary_image_storage_path");
                $this->uploadRepository->deleteImageFileInLocalPath($localFilePath,$URL);
                /**
                 * Empty the image_url and image_path field in the database.
                 */
    ...
    protected function updateGridQuery($bannerBuilder) {
        ...
        $bannerBuilder->with(['videos' => function ($query) {
            $query->selectRaw('videos.id, videos.title, videos.slug, videos.is_live,videos.is_archived,videos.is_kids,videos.is_active');
        }]);
        return $bannerBuilder->selectRaw('landingbanner.*, landingbanner.id as formatted_created_date');
    }
    ...
    public function getGridHeadings() {
        return [ StringLiterals::GRIDHEADING => [ 
            [ 'name' => "Banner title",'value' => 'name','sort' => false ],
            [ 'name' => trans ( 'cms::staticcontent.created_at' ),'value' => '','sort' => false ],
            // [ 'name' => trans ( 'cms::smstemplate.order' ),'value' => '','sort' => false ],
    ...
    public function getSearachVideo() {
        ...
        $bannerVideos = LandingBanner::where('is_active',1)->pluck('video_id');
        
        $video = $this->video->where( 'videos.is_active', '1' )
        ->where( 'videos.is_active', '1' )
        ->where( 'videos.is_kids', '1' )
