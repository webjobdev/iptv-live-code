...
class LiveBannerRepository extends BaseRepository {
     ...
     */
    public function __construct(LiveBanner $bannerContent, Video $video) {
        parent::__construct ();
    ...
    public function addOrUpdateBannerContents($id = null) { 
        ...
        $this->setMessages('banne_image.required', 'This image is required');
        ...
        if ($this->request->banner_image && $this->request->is_image_updated == 1) {
            ...
            $folderName = config("contus.base.image.live_image.s3_location");
            $localStoragePath = public_path().DIRECTORY_SEPARATOR.config("contus.base.image.live_image.temporary_image_storage_path");
         ...
         */
         if (!empty ($id)) {
            ...
            if(!empty($bannerImage)){
                ...
                $imageURL = config("contus.base.image.live_image.s3_location").$URL;
                $this->uploadRepository->deleteFileFromS3Bucket($imageURL);
                /** Process to delete image from local storage path */
                $localFilePath = public_path().DIRECTORY_SEPARATOR.config("contus.base.image.live_image.temporary_image_storage_path");
    ...
    protected function updateGridQuery($bannerBuilder) {
        ...
        return $bannerBuilder->selectRaw('livebanner.*, livebanner.id as formatted_created_date');
    }
    ...
    public function getGridHeadings() {
        return [ StringLiterals::GRIDHEADING => [ 
            [ 'name' => "Left side image",'value' => 'name','sort' => false ],
            [ 'name' => "Right side image",'value' => 'name','sort' => false ],
    ...
    public function getSearachVideo() {
        ...
        $bannerVideos = LiveBanner::where('is_active',1)->pluck('video_id');
