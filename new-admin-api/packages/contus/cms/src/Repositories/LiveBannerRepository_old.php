...
class LiveBannerRepository extends BaseRepository
{
    ...
    public function addOrUpdateBannerContents($id = null)
    {
        ...
        if ($this->request->banner_image && $this->request->is_image_updated == 1) {
            ...
            $folderName = config("contus.base.image.live_banner_image.s3_location");
            $localStoragePath = public_path() . DIRECTORY_SEPARATOR . config("contus.base.image.live_banner_image.temporary_image_storage_path");
    ...
    {
        return $this->_banner->where('is_active', 1)->select('banner_image', 'video_image', 'type', 'category_title', 'title', 'url', 'sort_no')->first();
    }
    /**
     * Prepare the grid
    ...
    protected function updateGridQuery($bannerBuilder)
    {
         ...
         */
        return $bannerBuilder->selectRaw('*, id as formatted_created_date');
    }
    ...
    public function recommendsearchVideos()
    {
        ...
        $video = $this->video->where('videos.is_active', '1')->where('job_status', 'Complete')->where('is_archived', 0)->where(function ($query) use ($searchKey) {
            $query->orwhere('slug', 'like', '%' . $searchKey . '%')->orwhere('title', 'like', '%' . $searchKey . '%')
                ->where('is_live', 0)->where('is_webseries', 0);
        });
