...
class KidsBannerRepository extends BaseRepository {
    ...
    public function addOrUpdateBannerContents($id = null) { 
        ...
        $bannerContent->video_title = $this->request->video_title?$this->request->video_title:''; 
        ...
        if ($this->request->mobile_image && $this->request->is_mobile_image_updated == 1) {
            ...
            $folderName = config("contus.base.image.mobile_bannerimage.s3_location");
            $localStoragePath = public_path().DIRECTORY_SEPARATOR.config("contus.base.image.mobile_bannerimage.temporary_image_storage_path");
            $s3BucketImgFilename = $this->uploadTos3Bucket($fileName,$folderName,$localStoragePath);
            $s3BucketImgURL = $folderName.$s3BucketImgFilename;
         ...
         */
         if (!empty ($id)) {
            ...
            if(!empty($bannerImage)){
                ...
                /** call to method to delete image in S3 bucket */
                $imageURL = config("contus.base.image.mobile_bannerimage.s3_location").$URL;
                $this->uploadRepository->deleteFileFromS3Bucket($imageURL);
                /** Process to delete image from local storage path */
                $localFilePath = public_path().DIRECTORY_SEPARATOR.config("contus.base.image.mobile_bannerimage.temporary_image_storage_path");
                $this->uploadRepository->deleteImageFileInLocalPath($localFilePath,$URL);
                /**
                 * Empty the image_url and image_path field in the database.
                 */
    ...
    protected function updateGridQuery($bannerBuilder) {
        ...
        return $bannerBuilder->selectRaw('kidsbanner.*, kidsbanner.id as formatted_created_date');
    }
    ...
    public function getSearachVideo() {
        ...
        $bannerVideos = KidsBanner::where('is_active',1)->pluck('video_id');
