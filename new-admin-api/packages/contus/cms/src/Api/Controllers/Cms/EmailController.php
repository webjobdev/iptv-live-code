...
class EmailController extends ApiController {
    ...
    public function getEmailData($id) {
        ...
        return ($data) ? $this->getSuccessJsonResponse ( [ 'response' => $data, 
            'language' => SiteLanguage::where('is_active', 1)->get()->toArray() ] ) : $this->getErrorJsonResponse ( [ ], trans ( 'customer::subscription.showError' ) );
    }
    ...
    public function addLanguage($id) {
        try {
            $this->repository->updateEmailTemplatesLanguage($id);
            return $this->getSuccessJsonResponse ( [ ],trans ( 'cms::emailtemplate.update.success' ));
        } catch (Exception $e) {
            return $this->getErrorJsonResponse ( [ ],trans ( 'cms::emailtemplate.update.error' )  );
        }
    }
