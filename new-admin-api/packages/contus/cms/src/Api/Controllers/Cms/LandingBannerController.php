...
use Contus\Cms\Repositories\LandingBannerRepository;
use Contus\Base\Repositories\UploadRepository;
use Contus\Base\Helpers\StringLiterals;

class LandingBannerController extends ApiController {
    /**
     * class property to hold the instance of BannerRepository
     *
     * @var \Contus\Base\Repositories\BannerRepository Construct method
     */
    public function __construct(LandingBannerRepository $bannerRepository, UploadRepository $uploadRepository) {
        parent::__construct ();
    ...
    public function postBannerImage() { 
        $tempImageInfo = $this->uploadRepository->setModelIdentifier ( 'web_image' )->tempPrepare ()->tempUpload ('web_image' );
    ...
    public function postMobileBannerImage() { 
        $tempImageInfo = $this->uploadRepository->setModelIdentifier ( 'mobile_image' )->tempPrepare ()->tempUpload ('mobile_image' );
        return empty ( $tempImageInfo ) ? $this->getErrorJsonResponse ( [ ], trans ( 'base::general.image_message.unable_to_upload' ) ) : $this->getSuccessJsonResponse ( [
                'info' => array_shift ( $tempImageInfo )
                ] );
    }
