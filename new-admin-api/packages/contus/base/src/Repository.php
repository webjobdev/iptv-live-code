...
abstract class Repository implements GridableRepository
{
    ...
    */
    ...
    {
        return ($this->authUser instanceof Authenticatable) ? \Auth::user()->id : 1;
    }
    ...
    public function getGridHeadings(){
        ...
        ['name' => trans('video::categories.category_name'), 'value' => 'title', 'sort' => true],        
        ['name' => trans('video::categories.no_of_videos'), 'value' => '', 'sort' => false], 
        ['name' => trans('video::categories.parent_category'), 'value' => '', 'sort' => false], 
        ['name' => trans('video::categories.status'), 'value' => 'is_active', 'sort' => false], 
        ['name' => 'order', 'value' => 'category_order', 'sort' => true], 
        ['name' => trans('video::categories.added_on'), 'value' => '', 'sort' => false], 
        ['name' => trans('video::categories.action'), 'value' => '', 'sort' => false]]];
    }
    /**
     * Method to get the the file name from 
     */
    public function getUploadedFilename($filename){
        $splitFilename = explode('/',$this->request->thumbnail);
        return $splitFilename [count($splitFilename) - 1];
    }
    /**
     * Method to upload to files to s3bucket
     * @param string $filename, actual file name
     * @param string $folderName, specify the folder to where the file shoule be stored in S3
     * @param string $localStoragePath, path where the image file is stored locally.
     * 
     * @return string file name
     */
    public function uploadTos3Bucket($fileName,$folderName,$localStoragePath){
        $URL = $filePath = '';
        $localFilePath = $localStoragePath.DIRECTORY_SEPARATOR.$fileName;
        $s3BucketFilename = $folderName.$fileName;
        try{
            $filePath = $this->awsRepository->uploadFileToS3($localFilePath,$s3BucketFilename);
            $URL = explode('/',$filePath);
            $AWSBucketURL = $URL[count($URL)-1];
        }
        catch (Exception $e){
     ...
     * @param $folderPermission interger
     */
    public function makeDirectory($directoryPath,$folderPermission){
        $newPath = str_replace(public_path(), '', $directoryPath);
        $newPath = ltrim($newPath, '/');
        $newArray = explode('/', $newPath);
        $destPath = public_path();
        foreach($newArray as $value) {
            $destPath .= DIRECTORY_SEPARATOR.$value;
            if (!is_dir($destPath)) {
                mkdir($destPath, $folderPermission, true);
                chmod($destPath, $folderPermission);
            }else{
                chmod($destPath, $folderPermission);
            }
        }
