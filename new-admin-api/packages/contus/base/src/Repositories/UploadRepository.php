...
class UploadRepository extends BaseUploadRepository implements IUploadRepository
{
    ...
    const MODEL_IDENTIFIER_ADMIN_USER_PROFILE = 'admin_user_profile_image';
    const MODEL_IDENTIFIER_CATEGORY_IMAGE = 'category_image';
    const MODEL_IDENTIFIER_MOBILE_CATEGORY_IMAGE = 'mobile_category_image';
    const MODEL_IDENTIFIER_SUBTITLE = 'subtitle';
    const MODEL_IDENTIFIER_POSTER = 'posters';
    const MODEL_IDENTIFIER_MOBILE_POSTER = 'mobile_posters';
    const MODEL_IDENTIFIER_CAST_IMAGE = 'cast_images';
    const MODEL_IDENTIFIER_ADS = 'ads';
    const MODEL_IDENTIFIER_LIVE_BANNER = 'live_banner_image';
    /**
    ...
    protected $allowedModelIdentifier = [
        self::MODEL_IDENTIFIER_THUMBNAIL,
        self::MODEL_IDENTIFIER_ADMIN_USER_PROFILE,
        self::MODEL_IDENTIFIER_CATEGORY_IMAGE,
        self::MODEL_IDENTIFIER_MOBILE_CATEGORY_IMAGE,
        self::MODEL_IDENTIFIER_SUBTITLE,
        self::MODEL_IDENTIFIER_POSTER,
        self::MODEL_IDENTIFIER_MOBILE_POSTER,
        self::MODEL_IDENTIFIER_CAST_IMAGE,
        self::MODEL_IDENTIFIER_LIVE_BANNER,
        'banner_image',
        'mobile_bannerimage',
        'mobile_image',
        'web_image',
    ...
    public function __construct()
    {
        ...
        //fix image resolution validation message error
        app(Factory::class)->replacer('resolution', function ($message, $attribute, $rule, $parameters) {
            return str_replace(':resolution', (isset($parameters[0])) ? $parameters[0] : '', ucfirst($message));
        });
     ...
     */
    public function tempUpload($moduleType = '', $imageSize = '')
    {
        $uploadedFiles = [];
        $responseImage = null;
        try {
            foreach ($this->uploadedFiles as $file) {
                ...
                $tempStoragePath = $this->path;
                /** Make base directory for image upload in public folder */
                $this->makeDirectory(public_path('uploads/tempimage'), 0777);
                /** Make temporary directory for image upload in public folder */
                $this->makeDirectory($tempStoragePath, 0777);
                if ($file->move($this->path, $fileName) && Makefile::exists($this->path . '/' . $fileName)) {
                    if ($moduleType == 'artist_image') {
                        $this->resizeImageAspectRatio($mime, $this->path, $fileName);
                    } else {
                        if ($moduleType === 'banner_image' || $moduleType === 'live_banner_image' || $moduleType === 'footer_image' || $moduleType !== 'category_image' || $moduleType !== 'mobile_category_image') {
                        } else {
                            $this->resizeImage($mime, $this->path, $fileName);
                        }
                    }
                }
                $TempImageFullPath = $this->path . '/' . $fileName;
                if (file_exists($TempImageFullPath)) {
                    chmod($TempImageFullPath, 0777);
                }
                if (!empty($moduleType) && ($moduleType == 'video-image' || $moduleType == 'channel-logo' || $moduleType == 'tvshow-image' || $moduleType == 'tvshow-season-image' || $moduleType == 'tvshow-season-episode-image' || $moduleType == 'vod-image' || $moduleType == 'album' || $moduleType == 'album_image')) {
                    $getTempStoragePath = (!empty($imageSize) && $imageSize == 'thumb')
                        ? config("contus.base.image.thumbnail.temporary_image_storage_path")
                        : config("contus.base.image.posters.temporary_image_storage_path");
                    $tempImageRelativePath = url('/') . '/' . $getTempStoragePath;
                    $responseImage = $tempImageRelativePath . '/' . $fileName;
                } elseif ($moduleType == 'profile') {
                    if ((!empty($imageSize)) && ($imageSize == 'thumb')) {
                        $getTempStoragePath = config("contus.base.image.admin_user_profile_image.temporary_image_storage_path");
                        $tempImageRelativePath = url('/') . '/' . $getTempStoragePath;
                        $responseImage = $tempImageRelativePath . '/' . $fileName;
                    } else {
                        $responseImage = $fileName;
                    }
                } else {
                    $responseImage = $fileName;
                }
                $uploadedFiles[] = $responseImage;
            }
        } catch (Exception $e) {
    ...
    /**
     * Method to delete the image form the localfile path
     * @param string $localStoragePath
     * @param string $filename
     *
     * @return void
     */
    public function deleteImageFileInLocalPath($localStoragePath, $fileName)
    {
        $localFilePath = $localStoragePath . DIRECTORY_SEPARATOR . $fileName;
        if (file_exists($localFilePath) && is_file($localFilePath)) {
            unlink($localFilePath);
        }
    }
    /**
     * Method to delete the file from amazon s3 bucket
     * @param string $filename
     */
    public function deleteFileFromS3Bucket($fileName)
    {
        return $this->awsRepository->deleteFileFromS3Bucket($fileName);
    }
    public function resizeImage($mime, $path, $fileName)
    {
        if (substr($mime, 0, 5) == 'image') {
            $img = Image::make($path . '/' . $fileName);
            if (isset(config('image')['dimensions'][$this->request->module])) {
                $dimensions = config('image')['dimensions'][$this->request->module][$this->request->size];
            } else {
                $dimensions = [550, 300];
            }
            $img->resize(intval($dimensions[0], $dimensions[1]), null, function ($constraint) {
                $constraint->aspectRatio();
            });
            if ($this->request->size == 'poster') {
    ...
    /**
     * Resize the image based on user required width & height
     *
     */
    public function resizeImageAspectRatio($file, $path, $fileName)
    {
        $img = Image::make($path . '/' . $fileName);
        $background = Image::canvas(200, 200);
        $resizeDimension = $this->getFileResizedAspectRatio(200, 200, $path, $fileName);
        $resizeWidth = $resizeDimension['width'];
        $resizeHeight = $resizeDimension['height'];
        $resizedImage = $img->resize($resizeWidth, $resizeHeight);
        $background->fill('#000000');
        // insert resized image centered into background
        $background->insert($resizedImage, 'center');
        $background->save($path . DIRECTORY_SEPARATOR . $fileName);
    }
    /**
     * Calculate the new width & height aspect ratio for given width & height based on input file
     *
     * @param int
     * @param int
     * @param object
     * @return array
     */
    public function getFileResizedAspectRatio($newwidth, $newheight, $path, $fileName)
    {
        ...
        return array('width' => ceil($resizeWidth), 'height' => ceil($resizeHeight));
    }
    /**
