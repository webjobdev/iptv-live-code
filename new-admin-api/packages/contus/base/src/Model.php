...
class Model extends IlluminateModel {
    ...
    public function setDynamicSlug($createSlugFrom, $thisSlug = 'slug') {
        $slug = Str::slug($this->$createSlugFrom);
        if ($slug && empty($this->$thisSlug)) {
            $count = (!empty($this->getKey()))
                ? $this->where($this->getKeyName(), '!=', $this->getKey())
                : $this;
            $count = $count->where($thisSlug, 'like', '%' . $slug . '%')->count();
     ...
     * @param string $thisSlug
     */
    public function generateDynamicSlug($video) {
        $slug = str_slug($video->title);
        if ($slug) {
            $count = Video::where('slug', 'like', '%' . $slug . '%')->where('id', '!=', $video->id)->count();
     ...
     * @param string
     */
    public function getImageBaseName($imgUrl) {
        if (!empty($imgUrl)) {
            $imagPathinfo = pathinfo($imgUrl);
            if (!empty($imagPathinfo['basename']) && !empty($imagPathinfo['extension'])) {
                return $imagPathinfo['basename'];
            }
        }
        return '';
    }
     ...
     * @param string
     */
    public function getImageBaseNameWithDirectory($imgUrl) {
        if (!empty($imgUrl)) {
            $imagPathinfo = pathinfo($imgUrl);
            if (!empty($imagPathinfo['basename']) && !empty($imagPathinfo['extension'])) {
                $splitDirectory = explode('/', $imagPathinfo['dirname']);
                $directory = $splitDirectory[count($splitDirectory) - 2] . '/' . $splitDirectory[count($splitDirectory) - 1];
                return $directory . '/' . $imagPathinfo['basename'];
            }
        }
        return '';
    }
    /**
     * Method to sort a query based on ID/ latest records
     * 
     * @param \Illuminate\Database\Eloquent\Builder $query
     * @param string $sortType
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function ScopeOrderByLatest($query, $sortType = 'DESC') {
        return $query->orderBy('id', $sortType);
    }
    /**
     * Method to sort a query based on order field
     * 
     * @param \Illuminate\Database\Eloquent\Builder $query
     * @param string $sortType
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function ScopeSortByOrderCol($query, $sortType = 'DESC') {
        return $query->orderBy('id', $sortType);
    }
}
