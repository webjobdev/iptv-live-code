...
trait GridHandler {
    ...
    public function getRecords() {
        ...
        try {
            ...
            $collection = (is_null ( $orderByFieldName ) || 
                          is_null ( $sortOrder )) ? 
                        $query->orderBy ( 'created_at', 'desc' )->paginate ( $pageLimit ) : $query->orderBy ( $orderByFieldName, $sortOrder )->paginate( $pageLimit );
        }
        catch ( QueryException $e ) {
            $this->logger->error ( $e->getMessage () );
        }
     ...
     * @throws \Illuminate\Database\Eloquent\ModelNotFoundException
     */
    public function gridUpdateStatus() {
        if ($this->request->has ( 'selectedCheckbox' ) && is_array ( $this->request->get ( 'selectedCheckbox' ) )) {
            $ids = $this->request->get ( 'selectedCheckbox' ) ;
            $status = $this->request->has ( 'status' ) ? $this->request->input ( 'status' ) : $isUpdated;
            if(empty($ids)){
                return false;
            }
            $ids = is_array ( $ids ) ? $ids : [ $ids ];
            if($this->gridModel instanceof \Contus\Video\Models\Video){
                $videoDetails  =array();
                $videoDetails['is_active'] = $status;
                $endpoint = 'add?_id='.$ids[0];
                foreach ($ids as $id){
                    PlaylistVideos::where('video_id' , (string)$id)->update(['is_active' => $status]);
                    FavouritesVideos::where('video_id' , (int)$id)->update(['is_active' => $status]);
                    $fav = FavouritesVideos::where('video_id' , (int)$id)->first();
                }
                $this->callElasticsearchService('PUT', $endpoint, $videoDetails);
            }
            return (!empty($status) && $status === 1) 
                ? $this->gridModel->whereIn('id', $ids)->update(['is_active' => 1])
                : $this->gridModel->whereIn('id', $ids)->update(['is_active' => 0]);
        }
    }
    /**
     * update mode
    ...
    /**
     * Method to handle the search service
     * 
     * @param string $method
     * @param string $endpoint
     * @param array $videoDetails
     */
    public function callElasticsearchService($method, $endpoint, $videoDetails){
        try{
            $client = new \GuzzleHttp\Client([
                'headers' => [ 'Content-Type' => 'application/json' ]
            ]);
            $URL = getenv ( 'SEARCH_URL' ).'search/elastic/'.$endpoint;
            $client->request($method, $URL ,['body' => json_encode($videoDetails)]);
