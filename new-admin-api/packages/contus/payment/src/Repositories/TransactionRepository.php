...
class TransactionRepository extends BaseRepository
{
    ...
    protected function updateGridQuery($transactionBuilder)
    {
        ...
        if (config()->get('auth.providers.users.table') === 'customers') {
            $transactionBuilder->where('customer_id', \Auth::user()->id);
        } 
        return $transactionBuilder->selectRaw('payment_transactions.*, payment_transactions.id as formatted_created_date, payment_transactions.id as formatted_end_date');
    }
            ...
            switch ($key) {
                ...
                case 'filter_end_date':
                    if ($key == 'filter_end_date') {                   
                      ...
                      $createdDate=Carbon::createFromFormat ( 'd-m-Y', $value )->format ( 'Y-m-d' );             
                      $builderTransaction =  $builderTransaction->whereDate('end_date','=', $createdDate);
    ...
    public function getGridHeadings()
    {
        ...
        [ 'name' => trans('payment::transaction.paid_amount') ,StringLiterals::VALUE => '','sort' => false,'class' => false ],
        [ 'name' => trans('payment::transaction.applied_coupon') ,StringLiterals::VALUE => '','sort' => false,'class' => false ],
        [ 'name' => trans('payment::transaction.status'),StringLiterals::VALUE => '','sort' => false,'class' => false ],
        [ 'name' => trans('payment::transaction.created_at'),StringLiterals::VALUE => '','sort' => false,'class' => false ],
        [ 'name' => "Expire Date",StringLiterals::VALUE => '','sort' => false,'class' => false ],
         ] ];
    }
