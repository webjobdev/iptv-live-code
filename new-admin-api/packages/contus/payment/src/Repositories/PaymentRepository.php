...
class PaymentRepository extends BaseRepository {
            ...
            if ($this->request->has ( 'setting' )) {
                foreach ( $paymentSet as $paymentsettings ) {
                    ...
                    if ($this->request->setting [$slug] && ! empty ( $this->request->setting [$slug] )) {
                        ...
                        $paymentsettings->updator_id = \Auth::user()->id;
    ...
    public function postccavRequestHandler($transactionamount,$orderId) {
        ...
        $data['merchant_param2'] = \Auth::user()->id;
    ...
    protected function updateGridQuery($paymentBuilder) {
         ...
         */
        if (\Auth::user()->id != 1) {
            $paymentBuilder->where ( 'id', \Auth::user()->id )->orWhere ( 'parent_id', \Auth::user()->id );
        }
    ...
    public function getGridHeadings() {
        ...
        [ 'name' => trans ( 'payment::payment.description' ),StringLiterals::VALUE => '','sort' => false ],[ 'name' => trans ( 'payment::payment.is_test' ),StringLiterals::VALUE => '','sort' => false ],[ 'name' => trans ( 'payment::payment.status' ),StringLiterals::VALUE => '','sort' => false ],[ 'name' => trans ( 'payment::payment.actions' ),StringLiterals::VALUE => '','sort' => false ] ] ];
    }
}