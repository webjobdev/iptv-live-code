...
class CustomerRepository extends BaseRepository
{
    ...
    public function __construct(Customer $customer, EmailTemplatesRepository $emailTemplates)
    {
        ...
        if (config()->get('auth.providers.users.table') === 'customers') {
            $this->setRules([ 
                ...
                StringLiterals::EMAIL => 'required|max:100|email|unique:customers,email,NULL,id,deleted_at,NULL',
                // 'phone' => 'nullable|numeric', 
                'phone' => 'required|max:15|min:7|unique:customers',
                'acesstype' => StringLiterals::REQUIRED,
                StringLiterals::PASSWORD => 'required|confirmed|min:6',
                'password_confirmation' => 'required | sometimes |same:password|min:6' 
            ]);
        } else {
            $this->setRules([ 
                'start_date' => 'sometimes|required',
                'subscription_plan' => 'sometimes|required',
                'orderid' => 'sometimes|required|numeric',
                ...
                'phone' => 'required|max:15|min:7|unique:customers',
                'acesstype' => StringLiterals::REQUIRED,
                StringLiterals::PASSWORD => 'required |sometimes|confirmed|min:6',
                'password_confirmation' => 'required| sometimes | same:password|min:6' 
            ]);
        }
    ...
    public function addOrUpdateCustomers($id = null)
    {
        ...
        if (! empty($id)) {
            ...
            // $this->setRule('phone', 'nullable|numeric');
            $this->setRule('phone','required|max:15|min:7|unique:customers,id,' . $customer->id);
            $this->setRule('acesstype', 'sometimes|required');
            $this->setRule('is_active', 'sometimes|required|boolean');
            $this->setRule('age', 'nullable|date|before:10 years ago');
            $this->_validate();
            $customer->updator_id = \Auth::user()->id;
            ...
            $customer->creator_id = (isset(\Auth::user()->id)) ? \Auth::user()->id : 0;
            if ($this->request->has('age') && $this->request->age) {
                $customer->dob = $this->request->age;
                $customer->age = Carbon::parse($this->request->age)->age;
            }
