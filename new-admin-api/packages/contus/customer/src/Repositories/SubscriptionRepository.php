...
class SubscriptionRepository extends BaseRepository {
    ...
    public function addOrUpdateSubscriptions($id = null) {
        if (! empty($id)) {
            ...
            $this->setRules([
                ...
                'amount' => 'sometimes|required|numeric|between:0,99999999.99',
                // 'amount_israel' => 'sometimes|required|numeric|between:0,99999999.99',
                'no_of_device' => 'required|numeric',
                'description' => 'nullable',
                'duration' => 'sometimes|required|numeric|min:1'
            ]);
            $subscription->updator_id = \Auth::user()->id;
        } else {
            $this->setRules([
                'name' => 'required|max:255',
                'type' => 'required|max:255',
                'amount' => 'required|numeric|between:0,99999999.99',
                // 'amount_israel' => 'required|numeric|between:0,99999999.99',
                'no_of_device' => 'required|numeric',
                'description' => 'nullable',
            ...
            $subscription->creator_id = Auth::user()->id;
        }
        $this->_validate();
        $subscription->fill($this->request->except('_token'));
        $subscription->description = ($this->request->has('description')) ? $this->request->description : '';
    ...
    public function addSubscriber($slug) {
        ...
        $email->subject = str_replace(['##SITE_NAME##'], [config()->get('settings.general-settings.site-settings.site_name')], $this->email->subject);
    ...
    public function prepareGrid() {
        if (config()->get('auth.providers.users.table') === 'customers') {
            ...
            $this->setGridModel($this->_subscription)->setEagerLoadingModels(['subscriptionTranslation']);
        }
        return $this;
    }
    ...
    protected function updateGridQuery($subscriptionBuilder) {
         ...
         */
        if (config()->get('auth.providers.users.table') === 'customers') {
            $subscriptionBuilder = $subscriptionBuilder->where('customer_id', \Auth::user()->id);
        }

        return $subscriptionBuilder->selectRaw('subscription_plans.*, subscription_plans.id as formatted_created_date');
    }
    ...
    public function getGridHeadings() {
        if (config()->get('auth.providers.users.table') === 'customers') {
            return [StringLiterals::GRIDHEADING => [
                ['name' => trans('customer::subscription.subscription_name'), StringLiterals::VALUE => '', 'sort' => true],
                ['name' => trans('customer::subscription.start_date'), StringLiterals::VALUE => '', 'sort' => false],

                ['name' => trans('customer::subscription.end_date'), StringLiterals::VALUE => '', 'sort' => true],
                ['name' => trans('customer::subscription.status'), StringLiterals::VALUE => 'is_active', 'sort' => false],
                ['name' => trans('customer::subscription.created_date'), StringLiterals::VALUE => '', 'sort' => true],
                ['name' => trans('customer::subscription.action'), StringLiterals::VALUE => '', 'sort' => false]
            ]];
        } else {
            return [StringLiterals::GRIDHEADING => [
                ['name' => trans('customer::subscription.subscription_name'), StringLiterals::VALUE => '', 'sort' => true, 'class' => false],

                ['name' => trans('customer::subscription.type'), StringLiterals::VALUE => '', 'sort' => false, 'class' => false],
                ['name' => trans('customer::subscription.amount') . ' ($)', StringLiterals::VALUE => '', 'sort' => false, 'class' => true],
                // ['name' => trans('customer::subscription.amount') . ' (₪)', StringLiterals::VALUE => '', 'sort' => false, 'class' => true],
                ['name' => trans('customer::subscription.duration'), StringLiterals::VALUE => '', 'sort' => false, 'class' => false],
                ['name' => trans('customer::subscription.created_at'), StringLiterals::VALUE => '', 'sort' => false, 'class' => false],
                ['name' => trans('customer::subscription.status'), StringLiterals::VALUE => 'is_active', 'sort' => false, 'class' => false],
                ['name' => trans('customer::subscription.action'), StringLiterals::VALUE => '', 'sort' => false, 'class' => false]
            ]];
        }
    }
    ...
    public function updateSubscribeTranslation($id) {
        if (!empty($id)) {
            $this->setRules(['name' => 'required', 'type' => 'required']);
            $this->validate($this->request, $this->getRules());

            $subscribeInfo = SubscriptionPlanTranslation::where('subscription_plan_id', '=', $id)->where('language_id', '=', $this->request->languageCode)->first();

            if (!empty($subscribeInfo)) {
                $category_translation = $subscribeInfo;
            } else {
                $category_translation = new SubscriptionPlanTranslation();
                $category_translation->subscription_plan_id = $id;
                $category_translation->language_id = $this->request->languageCode;
            }
            $category_translation->name = $this->request->name;
            $category_translation->type = $this->request->type;
            $category_translation->description = ($this->request->has('description') && !empty($this->request->description)) ? $this->request->description : '';
